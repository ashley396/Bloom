...
function orderCanCancel(status){const canon=canonicalOrderStatus(status);return !["CANCELLED","DELIVERED","COMPLETED"].includes(canon)}
...
if(t=e.target.closest("[data-cancel-order]")){const order=orders.find(x=>x.id===t.dataset.cancelOrder);if(!order||!orderCanCancel(order.status)||t.disabled)return;if(!confirm(`Cancel order ${order.order_number||""}?`))return;try{await api("orders",{method:"PATCH",body:JSON.stringify({id:order.id,status:"CANCELLED"})});toast("Order cancelled");await Promise.all([loadOrders(),loadDashboard(),loadInvoices()])}catch(err){toast(err.message)}return}
