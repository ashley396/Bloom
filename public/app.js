...
$("#staffForm").onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,d=Object.fromEntries(new FormData(f)),btn=$("#staffSaveButton");btn.disabled=true;btn.textContent="Saving…";try{const privateFields=["email","phone","hourly_rate","hire_date","federal_tax_rate","state_tax_rate","local_tax_rate","other_deduction_rate","fixed_deduction"];if(d.id){const patch={id:d.id,name:d.name,role:d.role};for(const key of privateFields)if(staffPrivateFieldFilled(key,d[key]))patch[key]=d[key];if(d.pin)patch.pin=d.pin;if(lastPrivateFilePin)patch.private_file_pin=lastPrivateFilePin;await api("staff",{method:"PATCH",body:JSON.stringify(patch)});if(d.pin)lastPrivateFilePin=d.pin}else{if(!/^\d{4,8}$/.test(String(d.pin||""))){toast("Employee PIN must be 4–8 digits.");return}const create={name:d.name,role:d.role,pin:d.pin};for(const key of privateFields)if(staffPrivateFieldFilled(key,d[key]))create[key]=d[key];await api("staff",{method:"POST",body:JSON.stringify(create)});lastPrivateFilePin=""}f.reset();$("#staffDialog").close();toast("Employee saved");await loadStaff()}catch(err){toast(err.message)}finally{btn.disabled=false;btn.textContent="Save employee"}};
