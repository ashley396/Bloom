...
$("#staffForm").onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,d=Object.fromEntries(new FormData(f)),btn=$("#staffSaveButton");btn.disabled=true;btn.textContent="Saving…";try{const privateFields=["email","phone","hourly_rate","hire_date","federal_tax_rate","state_tax_rate","local_tax_rate","other_deduction_rate","fixed_deduction"];if(d.id){const patch={id:d.id,name:d.name,role:d.role};for(const key of privateFields)if(d[key]!==""&&d[key]!=null)patch[key]=d[key];if(d.pin)patch.pin=d.pin;if(lastPrivateFilePin)patch.private_file_pin=lastPrivateFilePin;await api("staff",{method:"PATCH",body:JSON.stringify(patch)});if(d.pin)lastPrivateFilePin=d.pin}else{const created=await api("staff",{method:"POST",body:JSON.stringify({name:d.name,role:d.role,pin:d.pin})});const privatePatch={id:created.item.id,private_file_pin:d.pin};let hasPrivate=false;for(const key of privateFields){if(d[key]!==""&&d[key]!=null){privatePatch[key]=d[key];hasPrivate=true}}if(hasPrivate)await api("staff",{method:"PATCH",body:JSON.stringify(privatePatch)});lastPrivateFilePin=""}f.reset();$("#staffDialog").close();toast("Employee saved");await loadStaff()}catch(err){toast(err.message)}finally{btn.disabled=false;btn.textContent="Save employee"}};
