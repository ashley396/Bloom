...
function renderPosTiles(){const grid=$("#productPadGrid");if(!grid)return;if(!Array.isArray(posTiles)||!posTiles.length){try{posTiles=structuredClone(DEFAULT_POS_TILES)}catch{posTiles=[]}}renderPosCatTabs();const filter=$("#tileCategoryFilter")?.value||"all";const visible=(!filter||filter==="all")?posTiles:posTiles.filter(tile=>tile.category===filter);grid.innerHTML=visible.map(tile=>`<button class="pad quick-sale-pad${tile.image?" has-photo":""}" type="button" style="--cat:${posTileColor(tile.category)}${tile.image?`;background-image:url('${esc(tile.image)}')`:""}" data-tile-id="${esc(tile.id)}" data-sale-item="${esc(tile.name)}" title="${esc(tile.name)}" aria-label="${esc(tile.name)}"><span class="pad-name">${esc(tile.name)}</span></button>`).join("")+`<button class="pad pad-add" id="addTileFromGrid" type="button"><span class="pad-add-plus">＋</span>Add tile</button>`}
