...
["Blue Hydrangea Garden","Hydrangeas",89.99,"💙","A lush blue and white hydrangea arrangement with garden greenery.",[["Blue Hydrangea",4],["White Hydrangea",2],["Eucalyptus",5]],"https://images.pexels.com/photos/931177/pexels-photo-931177.jpeg?auto=compress&cs=tinysrgb&w=1000"],
...
function showSessionRecovery(message){
  ...
  const pub0=$("#publicHome");
  if(pub0)pub0.hidden=true;
...
function paymentBadgeClass(status){return String(status||"UNPAID").toUpperCase()==="PAID"?"good":"warn"}
...
const ORDER_STATUS_DEFS=[
  ...
  {id:"DESIGNING",label:"Designing",legacy:["DESIGNING"]},
  {id:"READY",label:"Ready",legacy:["READY"]},
  {id:"PICKUP_READY",label:"Pickup Ready",legacy:["PICKUP_READY"]},
  {id:"OUT_FOR_DELIVERY",label:"Out for Delivery",legacy:["OUT_FOR_DELIVERY"]},
  {id:"DELIVERED",label:"Delivered",legacy:["DELIVERED"]},
  {id:"COMPLETED",label:"Completed",legacy:["COMPLETED"]},
  {id:"ON_HOLD",label:"On Hold",legacy:["ON_HOLD"]},
  {id:"ISSUE",label:"Issue",legacy:["ISSUE"]},
  {id:"CANCELLED",label:"Cancelled",legacy:["CANCELLED"]}
];
const ORDER_BOARD_COLUMNS=[
  {id:"PENDING",label:"Pending",statuses:["NEW","PENDING","CONFIRMED"]},
  {id:"DESIGNING",label:"Designing",statuses:["DESIGNING"]},
  {id:"READY",label:"Ready",statuses:["READY","PICKUP_READY"]},
  ...
  {id:"ISSUE",label:"Issue",statuses:["ISSUE"]},
  {id:"CANCELLED",label:"Cancelled",statuses:["CANCELLED"]}
];
const ORDER_ADVANCE_SEQUENCE=["PENDING","CONFIRMED","DESIGNING","READY","PICKUP_READY","OUT_FOR_DELIVERY","DELIVERED","COMPLETED"];
function canonicalOrderStatus(value){const raw=String(value||"NEW").toUpperCase();const def=ORDER_STATUS_DEFS.find(s=>s.legacy.includes(raw)||s.id===raw);return def?.id||"PENDING"}
function boardColumnForStatus(status){const raw=String(status||"NEW").toUpperCase();const col=ORDER_BOARD_COLUMNS.find(c=>c.statuses.includes(raw));return col?.id||"PENDING"}
function orderStatusLabel(id){return ORDER_STATUS_DEFS.find(s=>s.id===id)?.label||id}
function nextAdvanceStatus(current){const canon=canonicalOrderStatus(current);const idx=ORDER_ADVANCE_SEQUENCE.indexOf(canon);if(idx<0||idx>=ORDER_ADVANCE_SEQUENCE.length-1)return null;return ORDER_ADVANCE_SEQUENCE[idx+1]}
function normalizeOrderStatus(value){return canonicalOrderStatus(value)}
function populateOrderStatusSelect(selected){const sel=$("#orderStatusSelect");if(!sel)return;sel.innerHTML=ORDER_STATUS_DEFS.map(s=>`<option value="${s.id}">${esc(s.label)}</option>`).join("");sel.value=canonicalOrderStatus(selected||"PENDING")}
async function loadOrderStatusHistory(orderId){const box=$("#orderStatusHistory"),section=$("#orderStatusHistorySection");if(!box||!orderId){section?.setAttribute("hidden","");return}section?.removeAttribute("hidden");box.innerHTML='<p class="subtle">Loading status history…</p>';try{const {items}=await api(`orders?order_id=${encodeURIComponent(orderId)}&view=history`);box.innerHTML=items?.length?items.map(row=>{const from=row.from_status?orderStatusLabel(canonicalOrderStatus(row.from_status)):"Start";const to=orderStatusLabel(canonicalOrderStatus(row.to_status));const when=new Date(row.created_at).toLocaleString();return `<div class="status-history-row"><strong>${esc(from)} → ${esc(to)}</strong><time datetime="${esc(row.created_at)}">${esc(when)}</time></div>`}).join(""):'<p class="subtle">No status changes recorded yet.</p>'}catch{box.innerHTML='<p class="subtle">Status history will appear after the Foundation migration is applied.</p>'}}
...
// A matching boutonniere is a second linked piece inside the SAME order
// (metadata.homecoming_prom.matching_boutonniere), not a second order — this
// badge is the "impossible to overlook" cue on the board itself, before a
// florist even opens the order.
function homecomingPieceBadge(o){return o?.metadata?.homecoming_prom?.matching_boutonniere?.needed?" · 🎓 2 PIECES (+boutonniere)":""}
function renderProductionOrder(o){
  const canon=canonicalOrderStatus(o.status),next=nextAdvanceStatus(o.status),nextLabel=next?orderStatusLabel(next):null,cogs=cogsSnapshot(o),pct=Math.round(cogs.ratio*100),meter=Math.min(100,pct);
  const pv=orderPaymentView(o);
  const guardrail=cogs.over?`<div class="cogs-warning">⚠ COGS is ${pct}% — above Florisyn's 33% target. Review the price or recipe before committing.</div>`:cogs.cost?`<div class="cogs-good">✓ COGS ${pct}% · within 33% target</div>`:`<div class="cogs-good">Add an estimated cost to activate the 33% guardrail.</div>`;
  const html=`<article class="production-card" data-order-card="${o.id}"><div class="card-top"><div><h3>${esc(o.order_number||"Order")} · ${esc(o.customer_name||"Customer")}</h3><div class="meta">${esc(orderStatusLabel(canon))} · ${esc(o.occasion||"Floral order")}${homecomingPieceBadge(o)} · ${dateText(o.delivery_date)} · Balance ${money(pv.balance)}</div></div><span class="badge ${paymentBadgeClass(pv.status)}">${esc(pv.status)}</span></div><div class="production-card-total"><strong>${money(cogs.total)}</strong><small>${o.recipient_name?`For ${esc(o.recipient_name)}`:esc(o.fulfillment||"")}</small></div><div class="cogs-meter ${cogs.over?"over":""}" title="COGS ${pct}%"><span style="width:${meter}%"></span></div>${guardrail}<div class="production-actions"><button class="secondary" data-edit-order="${o.id}">Quick edit</button><button class="secondary" data-manage-payment="${o.id}">Manage payment</button><button class="secondary" data-receipt="${o.id}">Receipt</button><button class="secondary danger" data-delete-order="${o.id}">Delete</button>${nextLabel?`<button class="primary advance-order" data-advance-order="${o.id}" data-current-status="${esc(o.status||"NEW")}">Advance → ${esc(nextLabel)}</button>`:`<button class="secondary" disabled>Completed</button>`}</div></article>`;
  return window.BloomRC21?.wrapProductionOrder?.(html,o)||html;
}
...
function invoiceCard(o){const pv=orderPaymentView(o);const paid=pv.status==="PAID";return `<article class="card invoice-card"><div class="card-top"><div><h3>Invoice ${esc(o.order_number||"")}</h3><div class="meta">${esc(o.customer_name||"Customer")} · ${dateText((o.created_at||"").slice(0,10))} · Paid ${money(pv.paid)} · Balance ${money(pv.balance)}</div></div><span class="badge ${paymentBadgeClass(pv.status)}">${esc(pv.status)}</span></div><p>${esc(o.recipient_name?`Recipient: ${o.recipient_name}`:(o.occasion||"Floral order"))}</p><div class="invoice-amount"><span>Total</span><strong>${money(o.total)}</strong></div><div class="card-actions"><button class="secondary" data-invoice-view="${o.id}" type="button">View / print</button><button class="secondary" data-invoice-email="${o.id}" type="button">Email invoice</button><button class="secondary" data-manage-payment="${o.id}" type="button">Manage payment</button>${paid?"":`<button class="primary" data-pay="${o.id}" type="button">Take payment</button>`}</div></article>`}
...
async function loadDashboard(){
  ...
  if($("#marginHealthScore"))$("#marginHealthScore").textContent=`${pi.marginHealth??"--"}%`;
  if($("#wasteRiskLabel"))$("#wasteRiskLabel").textContent=pi.wasteRisk||"Low";
  if($("#profitScoreMessage"))$("#profitScoreMessage").textContent=(pi.profitScore||100)>=85?"Excellent—your shop is protecting freshness and profit.":(pi.profitScore||0)>=65?"A few items need attention today.":"Act now on waste risk and low-margin work.";
  if($("#useFirstList"))$("#useFirstList").innerHTML=pi.useFirst?.length?pi.useFirst.map(i=>`<div class="use-first-row"><span class="freshness-dot ${i.freshness_score<=25?'danger':'warning'}"></span><div><strong>${esc(i.color?`${i.color} ${i.name}`:i.name)}</strong><small>Day ${i.age_days} · ${Number(i.quantity||0)} ${esc(i.unit||'stems')} left</small></div><b>${i.freshness_score}%</b></div>`).join(""):'<p class="subtle">Nothing urgent—your fresh inventory looks healthy.</p>';
  const special=pi.dailySpecial;window.bloomDailySpecial=special||null;
  if($("#dailySpecialName"))$("#dailySpecialName").textContent=special?.name||"No use-up special needed today";
  if($("#dailySpecialRecipe"))$("#dailySpecialRecipe").textContent=special?.items?.map(i=>`${i.quantity} ${i.color?i.color+' ':''}${i.name}`).join(" · ")||"Florisyn will suggest one when flowers begin aging.";
  if($("#dailySpecialWaste"))$("#dailySpecialWaste").textContent=money(special?.wastePrevented||0);
  if($("#createDailySpecial"))$("#createDailySpecial").disabled=!special;
  renderPosCart();
...
async function advanceOrderState(id,currentStatus){
  ...
  orderTelemetry(`UI advance ${id}: ${canonicalOrderStatus(currentStatus)} → ${next}`);
  try{
    orderTelemetry(`PATCH /.netlify/functions/orders { status: ${next} }`);
    const result=await api("orders",{method:"PATCH",body:JSON.stringify({id,status:next})});
    const updated=result.item||{};orders=orders.map(o=>o.id===id?{...o,...updated,status:updated.status||next}:o);renderOrderBoard();
    orderTelemetry(`Postgres orders UPDATE committed → ${next}`,"success");toast(`Order moved to ${orderStatusLabel(next)}`);
    loadDashboard();
...
async function loadDeliveries(){
  ...
  deliveries=items;
  const scheduled=items.filter(x=>!["OUT_FOR_DELIVERY","DELIVERED"].includes(x.status||"PENDING"));
  const out=items.filter(x=>(x.status||"")==="OUT_FOR_DELIVERY");
  const done=items.filter(x=>(x.status||"")==="DELIVERED");
  const miles=items.reduce((sum,x)=>sum+Number(x.round_trip_miles||0),0);
  if($("#deliveryScheduledCount")) $("#deliveryScheduledCount").textContent=scheduled.length;
  if($("#deliveryOutCount")) $("#deliveryOutCount").textContent=out.length;
  if($("#deliveryDoneCount")) $("#deliveryDoneCount").textContent=done.length;
  if($("#deliveryMilesTotal")) $("#deliveryMilesTotal").textContent=miles.toFixed(1);
  const list=$("#deliveriesList");
  if(!list)return;
  const card=x=>`<article class="card delivery-card"><div class="card-top"><div><h3>${esc(x.recipient_name||"Delivery stop")}</h3><div class="meta">${esc(x.delivery_date||"")} ${x.delivery_window?`· ${esc(x.delivery_window)}`:""}</div></div><span class="badge ${(x.status||"")==="DELIVERED"?"good":"warn"}">${esc(x.status||"PENDING")}</span></div><p>${esc(x.address||"No address entered")}</p>${x.driver?`<p class="subtle">Driver: ${esc(x.driver)}</p>`:""}${x.proof_signed_url?`<p class="subtle">Proof captured ${x.proof_captured_at?dateText(String(x.proof_captured_at).slice(0,10)):""}</p>`:""}<div class="card-actions">${(x.status||"")!=="DELIVERED"?`<button class="primary" data-advance-delivery="${x.id}" data-status="${esc(x.status||"PENDING")}">Advance status</button><button class="secondary" data-capture-proof="${x.id}">Capture proof</button>`:`<button class="secondary" data-view-proof="${x.id}">View proof</button>`}</div></article>`;
  list.innerHTML=items.length?items.map(card).join(""):empty("No delivery stops yet.");
}
async function loadInventory(){
  inventory=(await api("inventory")).items||[];
  const colors=[...new Set(inventory.map(i=>(i.color||"").trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
  const select=$("#inventoryColorFilter");
  if(select){const current=select.value;select.innerHTML='<option value="">All colors</option>'+colors.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join("");select.value=colors.includes(current)?current:""}
  if($("#inventoryColorTabs"))$("#inventoryColorTabs").innerHTML=['All',...colors].map(c=>`<button class="${(select?.value||"")===(c==='All'?'':c)?'active':''}" data-inventory-color="${esc(c==='All'?'':c)}">${esc(c)}</button>`).join("");
  renderInventory();
  window.BloomRC1?.enhanceInventoryCooler?.(inventory);
}
function renderInventory(){
  const q=($("#inventorySearch")?.value||"").trim().toLowerCase(),color=$("#inventoryColorFilter")?.value||"",freshFilter=$("#inventoryFreshnessFilter")?.value||"";
  const rows=inventory.filter(i=>!color||(i.color||"")===color).filter(i=>!freshFilter||inventoryFreshnessBucket(i)===freshFilter).filter(i=>[i.name,i.category,i.color,i.variety,i.lot_code,i.supplier].join(" ").toLowerCase().includes(q));
  const retail=rows.reduce((sum,i)=>sum+Number(i.quantity||0)*Number(i.price||0),0);if($("#inventoryRetailValue"))$("#inventoryRetailValue").textContent=money(retail);
  const listEl=$("#inventoryList");if(listEl)listEl.classList.add("bloom-inventory-visual");
  const helpers={inventoryFreshness};
  $("#inventoryList").innerHTML=rows.length?rows.map(i=>window.BloomRC21?.inventoryCard?.(i,helpers)||(()=>{const f=inventoryFreshness(i),freshClass=f.score<=25?"danger":f.score<=55?"warn":"good";return `<article class="card inventory-card${f.useFirst?" use-first-item":""}"><div class="card-top"><div><div class="inventory-title-row"><h3>${esc(i.name)}</h3>${i.color?`<span class="color-label">${esc(i.color)}</span>`:""}${f.useFirst?'<span class="badge warn">USE FIRST</span>':""}</div><div class="meta">${esc(i.category)}${i.variety?` · ${esc(i.variety)}`:""} · ${Number(i.quantity||0)} ${esc(i.unit)}</div></div><span class="badge ${Number(i.quantity)<=Number(i.low_stock_level)?"warn":"good"}">${Number(i.quantity)<=Number(i.low_stock_level)?"LOW":"IN STOCK"}</span></div><div class="freshness-meter"><div><span>Vase-life countdown</span><b class="${freshClass}">${f.label} · Day ${f.age}</b></div><progress max="100" value="${f.score}"></progress><small>${f.score}% freshness${i.received_at||i.arrival_date?` · Received ${dateText(i.received_at||i.arrival_date)}`:""}${i.use_by?` · Use by ${dateText(i.use_by)}`:""}${i.lot_code?` · Lot ${esc(i.lot_code)}`:""}${i.supplier?` · ${esc(i.supplier)}`:""}${i.markup_multiplier?` · ${Number(i.markup_multiplier)}× markup`:""}</small></div><div class="inventory-pricing"><span>Wholesale <strong>${money(i.cost)}</strong></span><span>Retail <strong>${money(i.price)}</strong></span><span>Stock retail value <strong>${money(Number(i.quantity||0)*Number(i.price||0))}</strong></span></div><div class="card-actions"><button class="secondary" data-edit-inventory="${i.id}">Edit</button><button class="secondary" data-delete-inventory="${i.id}">Delete</button></div></article>`})()).join(""):empty("No inventory matches this view.")
}
async function loadProducts(){products=(await api("products")).items||[];window.__bloomProducts=products;window.BloomRC2?.mountRecipePrivacy?.();$("#productsList").innerHTML=products.length?products.map(productCard).join(""):empty("Create your first product.");$("#orderProduct").innerHTML='<option value="">Custom arrangement</option>'+products.map(p=>`<option value="${p.id}" data-price="${p.price}" data-cost="${p.estimated_cost||p.labor_cost||0}">${esc(p.name)} · ${money(p.price)}</option>`).join("")}
...
function marketplaceVerificationRules(){return window.BloomMarketplaceVerification?.getVerificationRules?window.BloomMarketplaceVerification.getVerificationRules():{requiredFields:["legal_name","owner_name","business_address","phone","email","sales_tax_permit_number"],requiredDocuments:["resale_certificate","government_id"]};}
const MARKETPLACE_VERIFICATION_KEY="bloom_marketplace_verification_profile";
const MARKETPLACE_VERIFICATION_RULES=marketplaceVerificationRules();
let marketplaceVerificationProfile=null,marketplaceVerificationStep=0;

function loadMarketplaceVerificationProfile(){try{const saved=JSON.parse(localStorage.getItem(MARKETPLACE_VERIFICATION_KEY)||"null");marketplaceVerificationProfile=window.BloomMarketplaceVerification?.normalizeMarketplaceVerificationProfile?window.BloomMarketplaceVerification.normalizeMarketplaceVerificationProfile(saved&&typeof saved==="object"?saved:{status:"not_started",documents:{}}):{status:"not_started",documents:{}}}catch{marketplaceVerificationProfile=window.BloomMarketplaceVerification?.normalizeMarketplaceVerificationProfile?window.BloomMarketplaceVerification.normalizeMarketplaceVerificationProfile({status:"not_started",documents:{}}):{status:"not_started",documents:{}}}return marketplaceVerificationProfile}
function saveMarketplaceVerificationProfile(){marketplaceVerificationProfile=window.BloomMarketplaceVerification?.normalizeMarketplaceVerificationProfile?window.BloomMarketplaceVerification.normalizeMarketplaceVerificationProfile(marketplaceVerificationProfile||{}):marketplaceVerificationProfile||{};localStorage.setItem(MARKETPLACE_VERIFICATION_KEY,JSON.stringify(marketplaceVerificationProfile));renderMarketplaceVerificationCard();}
async function loadMarketplaceVerificationApplicationFromServer(){if(!session?.accessToken)return null;try{const result=await api("marketplace-verification",{method:"GET"});const application=result.application;const profileData=application?.profile_data&&typeof application.profile_data==="object"?application.profile_data:{};if(application){const profile=window.BloomMarketplaceVerification?.normalizeMarketplaceVerificationProfile?window.BloomMarketplaceVerification.normalizeMarketplaceVerificationProfile({...profileData,status:application.status||profileData.status||"draft",review_history:Array.isArray(application.review_history)?application.review_history:profileData.review_history,consent_confirmed:typeof application.consent_confirmed==="boolean"?application.consent_confirmed:profileData.consent_confirmed}):{status:"not_started",documents:{}};marketplaceVerificationProfile=profile;localStorage.setItem(MARKETPLACE_VERIFICATION_KEY,JSON.stringify(profile));renderMarketplaceVerificationCard();return profile}}catch{}return null}
async function syncMarketplaceVerificationApplicationToServer(profile,options={}){const helper=window.BloomMarketplaceVerification||{};if(!session?.accessToken)return null;const normalized=helper.normalizeMarketplaceVerificationProfile?helper.normalizeMarketplaceVerificationProfile(profile||{}):profile||{};const payload=helper.buildMarketplaceVerificationSubmissionPayload?helper.buildMarketplaceVerificationSubmissionPayload(normalized,{...options,userId:session.user?.id}):{profile_data:normalized};const result=await api("marketplace-verification",{method:"POST",body:JSON.stringify({profile_data:payload,submit:Boolean(options.submit),status:options.status||payload.status})});const application=result.application;const profileData=application?.profile_data&&typeof application.profile_data==="object"?application.profile_data:{};if(application){const nextProfile=helper.normalizeMarketplaceVerificationProfile?helper.normalizeMarketplaceVerificationProfile({...profileData,status:application.status||normalized.status||"draft",review_history:Array.isArray(application.review_history)?application.review_history:normalized.review_history,consent_confirmed:typeof application.consent_confirmed==="boolean"?application.consent_confirmed:normalized.consent_confirmed}):{...normalized,status:application.status||normalized.status||"draft"};marketplaceVerificationProfile=nextProfile;localStorage.setItem(MARKETPLACE_VERIFICATION_KEY,JSON.stringify(nextProfile));renderMarketplaceVerificationCard();return nextProfile}return null}
function getMarketplaceVerificationStatus(){const helper=window.BloomMarketplaceVerification||{};return helper.getVerificationStatus?helper.getVerificationStatus(marketplaceVerificationProfile||{}):"not_started"}
function getMarketplaceVerificationProgress(){const helper=window.BloomMarketplaceVerification||{};return helper.buildVerificationProgress?helper.buildVerificationProgress(marketplaceVerificationProfile||{},marketplaceVerificationRules()):{percent:0,missingFields:[],missingDocuments:[]}}
function renderMarketplaceVerificationCard(){const profile=loadMarketplaceVerificationProfile();const progress=getMarketplaceVerificationProgress();const status=getMarketplaceVerificationStatus();const statusLabel=$("#verificationStatusLabel");const statusText=$("#verificationStatusText");const statusMap={not_started:"Not started",draft:"Draft",submitted:"Submitted",under_review:"Under review",more_info_required:"More information required",approved:"Approved",rejected:"Rejected",expired:"Expired",suspended:"Suspended"};if(statusLabel)statusLabel.textContent=statusMap[status]||"Not started";if(statusText){const messages={not_started:"Start a business profile to unlock purchasing and document review.",draft:`${progress.percent}% complete. Your progress is saved automatically so you can return later.`,submitted:"Your application is waiting for review from the wholesaler.",under_review:"The wholesaler is reviewing your business profile and documents.",more_info_required:`Action needed: ${progress.missingFields.join(", ")||progress.missingDocuments.join(", ")||"provide the missing items listed below."}`,approved:"Your profile is approved for verified wholesale purchasing.",rejected:"Your application was rejected. Review the notes and correct the issue before reapplying.",expired:"This profile has expired. Renew the required documents to continue purchasing.",suspended:"Purchasing is currently suspended until the issue is resolved."};statusText.textContent=messages[status]||messages.not_started;}const progressBar=$("#verificationProgressBar span");if(progressBar)progressBar.style.width=`${Math.max(8,progress.percent)}%`;}
...
$("#marketplaceVerificationForm")?.addEventListener("submit",handleMarketplaceVerificationSubmit);
renderMarketplaceVerificationCard();
loadMarketplaceVerificationApplicationFromServer();

document.addEventListener("click",e=>{const button=e.target.closest("[data-market-request]");if(!button)return; e.preventDefault(); if(getMarketplaceVerificationStatus()!=="approved"){openMarketplaceVerificationDialog();toast("Complete your business verification before creating a wholesale purchase request.");return;} toast("Purchase request prepared for the selected wholesaler.");});

async function loadMarketplace(){const categories=window.BloomMarketplaceCategories?.MARKETPLACE_CATEGORIES||[];const listingCategory=$("#marketplaceListingCategory");if(listingCategory&&categories.length)listingCategory.innerHTML=categories.map(c=>`<option>${esc(c.label)}</option>`).join("");const wholesaleCategoryList=$("#wholesaleCategoryList");if(wholesaleCategoryList&&categories.length)wholesaleCategoryList.innerHTML=categories.map(c=>`<option value="${esc(c.label)}">`).join("");if(window.BloomMarketplaceExperience?.load){await window.BloomMarketplaceExperience.load({api,esc,money,empty,toast,$,getVerificationStatus:getMarketplaceVerificationStatus,openVerificationDialog:openMarketplaceVerificationDialog});return;}const a=(await api("marketplace")).items||[];$("#marketplaceBrowseGrid")&&( $("#marketplaceBrowseGrid").innerHTML=a.length?a.map(x=>`<article class="product-card">${x.image_url?`<img src="${esc(x.image_url)}">`:`<div class="product-art">🌹</div>`}<div class="body"><span class="badge">${esc(x.category)}</span><h3>${esc(x.product_name)}</h3><p>${esc(x.supplier_name)}</p><div class="price">${money(x.price)} / ${esc(x.unit)}</div><p class="meta">Minimum ${x.minimum_quantity}</p><button class="secondary wide" data-market-request="${x.id}">Create purchase request</button></div></article>`).join(""):empty("No wholesale listings."));}
async function loadWholesaleSeller(){window.BloomLaunchPolish?.mountOnboarding?.("wholesaler");if(!window.BloomWholesaleSeller?.load){toast("Seller dashboard is still loading.");return;}await window.BloomWholesaleSeller.load({api,esc,money,empty,toast,$,openVerificationDialog:openMarketplaceVerificationDialog,runAiDescription:async(target)=>{const button=document.createElement("button");button.dataset.aiTarget=target.id;button.dataset.aiKind="wholesale product description";await runContentHelper(button);}});}
$("#wholesaleNewProductBtn")?.addEventListener("click",()=>{showPage("wholesaleSellerPage");setTimeout(()=>window.BloomWholesaleSeller?.openProductDialog?.({api,esc,money,empty,toast,$},null),0);});
function filteredExpenses(){const q=($("#expenseSearch")?.value||"").trim().toLowerCase(),category=$("#expenseCategoryFilter")?.value||"",month=$("#expenseMonthFilter")?.value||"";return expenses.filter(x=>(!category||x.category===category)&&(!month||String(x.expense_date||"").startsWith(month))&&(!q||[x.vendor,x.category,x.notes,x.amount].join(" ").toLowerCase().includes(q)))}
function renderExpenses(){const rows=filteredExpenses(),total=rows.reduce((sum,x)=>sum+Number(x.amount||0),0);if($("#expenseVisibleTotal"))$("#expenseVisibleTotal").textContent=money(total);if($("#expenseVisibleCount"))$("#expenseVisibleCount").textContent=String(rows.length);$("#expensesList").innerHTML=rows.length?rows.map(x=>`<article class="card"><div class="card-top"><div><h3>${esc(x.vendor||x.category||"Expense")}</h3><div class="meta">${dateText(x.expense_date)} · ${esc(x.category||"Other")}</div></div><strong>${money(x.amount)}</strong></div>${x.notes?`<p>${esc(x.notes)}</p>`:""}${x.receipt_url?`<p><a class="secondary receipt-link" href="${esc(x.receipt_url)}" target="_blank" rel="noopener">Open receipt</a></p>`:`<p class="subtle">No receipt attached</p>`}<div class="card-actions"><button type="button" class="secondary" data-edit-expense="${x.id}">Edit expense</button><button type="button" class="secondary danger" data-delete-expense="${x.id}">Delete expense</button></div></article>`).join(""):empty("No expenses match this view.")}
...
function applyShopHoursRow(weekday,row){const closedBox=$(`#hoursClosed${weekday}`),openInput=$(`#hoursOpen${weekday}`),closeInput=$(`#hoursClose${weekday}`),rowEl=document.querySelector(`.hours-row[data-weekday="${weekday}"]`);if(!closedBox||!openInput||!closeInput)return;closedBox.checked=Boolean(row.is_closed);openInput.value=row.opens_at||"09:00";closeInput.value=row.closes_at||"17:00";rowEl?.classList.toggle("hours-row-closed",Boolean(row.is_closed))}
async function loadShopHours(){try{window.shopHours=(await api("shop-hours")).hours;(window.shopHours||[]).forEach(row=>applyShopHoursRow(row.weekday,row))}catch(e){if($("#shopHoursStatus"))$("#shopHoursStatus").textContent=e.message||"Could not load store hours."}}
async function saveShopHours(){const hours=HOURS_WEEKDAY_SHORT.map((_,weekday)=>({weekday,is_closed:$(`#hoursClosed${weekday}`)?.checked||false,opens_at:$(`#hoursOpen${weekday}`)?.value||"09:00",closes_at:$(`#hoursClose${weekday}`)?.value||"17:00"}));const status=$("#shopHoursStatus");try{if(status)status.textContent="Saving…";window.shopHours=(await api("shop-hours",{method:"PUT",body:JSON.stringify({hours})})).hours;if(status)status.textContent="Store hours saved.";toast("Store hours saved")}catch(e){if(status)status.textContent=e.message||"Could not save store hours.";toast(e.message||"Could not save store hours.")}}
...
async function addLibraryProduct(index=null){let payload;if(index!==null&&index!==undefined){const p=LIBRARY[Number(index)];payload={name:p[0],category:p[1],price:p[2],description:p[4],image_url:p[6]||"",items:p[5].map(r=>({ingredient_name:r[0],quantity:r[1],unit:"stem",unit_cost:0}))}}else payload=libraryDialogPayload();if(!payload.name)return toast("Enter a product name first");const {item}=await api("products",{method:"POST",body:JSON.stringify({...payload,available_online:true,active:true,featured:true})});await api("recipes",{method:"POST",body:JSON.stringify({product_id:item.id,items:payload.items})});$("#libraryDesignDialog")?.close();await loadProducts();toast(`${payload.name} was added to Products & Recipe Builder`)}
...
function openReceipt(o){if(!o)return;openReceiptWithPayments(o)}
async function openReceiptWithPayments(o){if(!o)return;const pv=orderPaymentView(o);let payments=[];try{payments=(await api(`payments?order_id=${encodeURIComponent(o.id)}`)).items||[]}catch{}const mark=shopSettings?.logo_url?"":`<img class="receipt-florisyn-mark" src="/assets/florisyn/florisyn-monochrome.svg" alt="" width="40" height="40">`;const taxPct=Number(o.tax_rate||0);const itemsBlock=o.arrangement_description?`<div class="receipt-line receipt-items"><span>Items</span><span>${esc(o.arrangement_description)}</span></div>`:"";const tenderBlock=payments.length?`<div class="receipt-tenders"><p class="receipt-label">Payments</p>${payments.map(p=>{const label=p.metadata?.tender_label||p.method;const note=p.metadata?.note?` · ${esc(p.metadata.note)}`:"";return `<div class="receipt-line"><span>${esc(label)}${note}</span><span>${money(p.amount)}</span></div>`}).join("")}</div>`:(o.payment_method?`<div class="receipt-line"><span>Last method</span><span>${esc(o.payment_method)}</span></div>`:"");$("#receiptContent").innerHTML=`<div id="receiptPrintRoot"><div class="receipt receipt-compact"><div class="receipt-brand">${shopSettings?.logo_url?`<img class="print-logo" src="${esc(shopSettings.logo_url)}" alt="">`:mark}<h1>${shopSettings?.logo_url?"":""}${esc(shopSettings?.name||"Florisyn")}</h1><p>${esc(shopSettings?.address||"Beautiful flowers, thoughtfully made")}</p>${shopSettings?.phone?`<p>${esc(shopSettings.phone)}</p>`:""}</div><div class="receipt-head"><strong>${esc(o.order_number)}</strong><strong>${new Date(o.created_at).toLocaleDateString()}</strong></div><div class="receipt-grid receipt-grid-compact"><div><strong>${esc(o.customer_name)}</strong>${o.recipient_name?`<p>${esc(o.recipient_name)}</p>`:""}</div><div><strong>${esc(o.fulfillment)}</strong><p>${dateText(o.delivery_date)}</p></div></div>${itemsBlock}<div class="receipt-line"><span>Subtotal</span><span>${money(o.subtotal)}</span></div><div class="receipt-line"><span>Tax${taxPct?` (${taxPct}%)`:""}</span><span>${money(o.tax)}</span></div><div class="receipt-line"><span>Delivery</span><span>${money(o.delivery_fee)}</span></div><div class="receipt-total-row"><span>Total</span><span>${money(o.total)}</span></div>${tenderBlock}<div class="receipt-line"><span>Paid</span><span>${money(pv.paid)}</span></div><div class="receipt-line"><span>Balance due</span><span>${money(pv.balance)}</span></div><div class="payment-stamp">${esc(pv.status)}</div>${o.card_message?`<p class="receipt-card-msg">${esc(o.card_message)}</p>`:""}<p class="receipt-footer receipt-thanks">${shopSettings?.receipt_header?esc(shopSettings.receipt_header):"Thank you for supporting a local flower shop."}</p></div></div>`;$("#receiptDialog").showModal()}
function printReceiptOnly(){document.body.classList.add("print-receipt-mode");const cleanup=()=>{document.body.classList.remove("print-receipt-mode");window.removeEventListener("afterprint",cleanup)};window.addEventListener("afterprint",cleanup);window.print()}
async function openPaymentCenterForOrder(order){if(!order?.id){toast("Order not found");return}paymentReturnOrderId=order.id;setPendingPaymentOrder(order);syncLegacyPaymentFields(order);if($("#paymentStatus"))$("#paymentStatus").textContent="";showPage("paymentsPage");loadSplitSessionFromStorage();if(splitPaymentSession?.orderId===order.id&&orderPaymentView(order).balance>0.005){setPaymentCenterMode("split");renderSplitPaymentRows(defaultSplitRowsForRemaining())}else setPaymentCenterMode("methods");await applyPaymentHubCheckout();await refreshPaymentHistory()}
function setAuthMode(isCreate){
  ...
  if(authButton)authButton.textContent=createMode?"Create my Florisyn account":"Sign in";
  const switchPrompt=$("#switchPrompt");
  if(switchPrompt)switchPrompt.textContent=createMode?"Already have a Florisyn account?":"New to Florisyn?";
...
function loadSavedQuote(id){const q=savedQuotes.find(x=>x.id===id);if(!q)return;posCart=structuredClone(q.cart||[]);if($("#posCustomerSelect"))$("#posCustomerSelect").value=q.customerId||"";if($("#posOrderNote"))$("#posOrderNote").value=q.note||"";savePosCart();$("#savedQuotesDialog")?.close();toast(`Loaded ${q.name}`)}
function syncLegacyPaymentFields(order){const{balance}=orderPaymentView(order||pendingPaymentOrder||{});const amt=balance>0?balance.toFixed(2):"0.00";if($("#paymentAmount"))$("#paymentAmount").value=amt;if($("#cardPaymentAmount"))$("#cardPaymentAmount").value=amt;if($("#cashPaymentAmount"))$("#cashPaymentAmount").value=amt;if($("#cashReceived"))$("#cashReceived").value=amt;if($("#checkPaymentAmount"))$("#checkPaymentAmount").value=amt;if($("#paymentDescription"))$("#paymentDescription").value=`${order?.order_number||pendingPaymentOrder?.order_number||"Florisyn order"} - ${order?.customer_name||pendingPaymentOrder?.customer_name||"Customer"}`}
function getPaymentBalance(){return orderPaymentView(pendingPaymentOrder||{}).balance}
function renderPaymentCenterShell(){const empty=$("#paymentCenterEmpty"),active=$("#paymentCenterActive");if(!pendingPaymentOrder?.id){if(empty)empty.hidden=false;if(active)active.hidden=true;renderPendingPayment();return}if(empty)empty.hidden=true;if(active)active.hidden=false;renderPaymentTopSummary();renderPendingPayment()}
function renderPaymentTopSummary(){const box=$("#paymentTopSummary");if(!box||!pendingPaymentOrder)return;const pv=orderPaymentView(pendingPaymentOrder);box.dataset.balance=String(pv.balance);box.innerHTML=`<p class="pay-customer">${esc(pendingPaymentOrder.customer_name||"Customer")}</p><p class="pay-order-meta">Order ${esc(pendingPaymentOrder.order_number||"—")}</p><div class="pay-stats"><div><span>Order total</span><strong>${money(pv.total)}</strong></div><div><span>Already paid</span><strong>${money(pv.paid)}</strong></div><div class="pay-balance-block"><span>Balance due</span><strong>${money(pv.balance)}</strong></div><div><span>Status</span><strong><span class="badge ${paymentBadgeClass(pv.status)}">${esc(pv.status)}</span></strong></div></div>`}
function renderPendingPayment(){const box=$("#pendingPaymentSummary"),full=$("#useFullBalance");if(!box)return;if(!pendingPaymentOrder){box.innerHTML="";if(full)full.disabled=true;return}const pv=orderPaymentView(pendingPaymentOrder);box.dataset.balance=String(pv.balance);box.innerHTML=`<span data-legacy-summary>${money(pv.balance)}</span>`;if(full)full.disabled=pv.balance<=0}
function hideAllPaymentFlows(){["paymentFlowCash","paymentFlowCard","paymentFlowCheck","paymentFlowSplit","paymentFlowPayLater","paymentSuccessPanel"].forEach(id=>{const el=$("#"+id);if(el)el.hidden=true});$("#paymentMethodPicker")?.removeAttribute("hidden");document.querySelector(".payment-more-methods")?.removeAttribute("hidden")}
function setPaymentCenterMode(mode){hideAllPaymentFlows();$("#splitPaymentError")&&( $("#splitPaymentError").textContent="");const picker=$("#paymentMethodPicker"),more=document.querySelector(".payment-more-methods");if(mode==="methods"){picker?.removeAttribute("hidden");more?.removeAttribute("hidden");return}picker&&(picker.hidden=true);more&&(more.hidden=true);if(mode==="cash"){const bal=getPaymentBalance();$("#cashPaymentAmount").value=bal>0?bal.toFixed(2):"0.00";$("#cashReceived").value=bal>0?bal.toFixed(2):"0.00";updateCashMath();$("#paymentFlowCash").hidden=false;$("#cashPaymentAmount")?.focus()}else if(mode==="card"){const bal=getPaymentBalance();$("#cardPaymentAmount").value=bal>0?bal.toFixed(2):"0.00";syncLegacyPaymentFields(pendingPaymentOrder);$("#paymentFlowCard").hidden=false}else if(mode==="check"){const bal=getPaymentBalance();$("#checkPaymentAmount").value=bal>0?bal.toFixed(2):"0.00";const btn=$("#completeCheckPayment");if(btn)btn.dataset.moreMethod="Check";const h2=$("#paymentFlowCheck")?.querySelector("h2");if(h2)h2.textContent="Check payment";$("#paymentFlowCheck").hidden=false}else if(mode==="split"){ensureSplitSession();if(!$("#splitPaymentRows")?.querySelector(".split-part-row"))renderSplitPaymentRows(defaultSplitRowsForRemaining());else updateSplitTotals();$("#paymentFlowSplit").hidden=false}else if(mode==="pay-later"){$("#paymentFlowPayLater").hidden=false}else if(mode==="success"){$("#paymentSuccessPanel").hidden=false}}
function updateCashMath(){const pay=Number($("#cashPaymentAmount")?.value||0),received=Number($("#cashReceived")?.value||0),balance=getPaymentBalance();const change=Math.max(0,Math.round((received-pay)*100)/100);const after=Math.max(0,Math.round((balance-pay)*100)/100);if($("#cashChangeDue"))$("#cashChangeDue").textContent=money(change);if($("#cashBalanceAfter"))$("#cashBalanceAfter").textContent=money(after);$("#paymentAmount").value=pay>0?pay.toFixed(2):"0.00"}
function returnFromPaymentCenter(){clearSplitSession();const id=paymentReturnOrderId||pendingPaymentOrder?.id;paymentReturnOrderId=null;showPage("ordersPage");if(id){const card=document.querySelector(`[data-order-card="${id}"]`);card?.scrollIntoView({behavior:"smooth",block:"nearest"})}}
function cancelPaymentCenter(){clearSplitSession();setPaymentCenterMode("methods");if($("#paymentStatus"))$("#paymentStatus").textContent=""}
function loadSplitSessionFromStorage(){try{splitPaymentSession=JSON.parse(sessionStorage.getItem("bloom_split_payment_session")||"null")}catch{splitPaymentSession=null}return splitPaymentSession}
function saveSplitSession(){if(splitPaymentSession)sessionStorage.setItem("bloom_split_payment_session",JSON.stringify(splitPaymentSession));else sessionStorage.removeItem("bloom_split_payment_session")}
function clearSplitSession(){splitPaymentSession=null;sessionStorage.removeItem("bloom_split_payment_session");sessionStorage.removeItem("bloom_split_stripe_return")}
function ensureSplitSession(){loadSplitSessionFromStorage();if(!pendingPaymentOrder?.id){splitPaymentSession=null;saveSplitSession();return null}if(!splitPaymentSession||splitPaymentSession.orderId!==pendingPaymentOrder.id){splitPaymentSession={orderId:pendingPaymentOrder.id,batchId:(crypto.randomUUID?crypto.randomUUID():`split-${Date.now()}`),partSeq:0};saveSplitSession()}return splitPaymentSession}
function defaultSplitRowsForRemaining(){const balance=getPaymentBalance();const half=balance>0?Math.round(balance*50)/100:0;return splitPaymentSession?.partSeq>0||balance<=0?[{method:"Cash",amount:balance,note:""}]:[{method:"Cash",amount:half,note:""},{method:"Card",amount:Math.max(0,Math.round((balance-half)*100)/100),note:""}]}
function continueSplitWorkflow(order,statusMessage){ensureSplitSession();renderSplitWorkflowStep(order,statusMessage)}
function renderSplitWorkflowStep(order,statusMessage){if(order)setPendingPaymentOrder(order);renderPaymentCenterShell();setPaymentCenterMode("split");renderSplitPaymentRows(defaultSplitRowsForRemaining());if($("#paymentStatus")&&statusMessage)$("#paymentStatus").textContent=statusMessage;refreshPaymentHistory()}
async function completeSplitWorkflow(order){clearSplitSession();if(order)setPendingPaymentOrder(order);await refreshPaymentHistory();showPaymentSuccessPanel({detail:`Split payment complete. Balance ${money(orderPaymentView(order||pendingPaymentOrder).balance)}.<br>Print receipt to show every tender.`,order:order||pendingPaymentOrder});toast("Split payment complete")}
async function handleSplitPaymentContinuation(result,deferred=[]){const order=result.order||pendingPaymentOrder;if(order)setPendingPaymentOrder(order);await Promise.all([loadOrders(),loadDashboard(),loadInvoices()]);const balance=orderPaymentView(order).balance;if(deferred?.length){const amt=Math.min(Number(deferred[0].amount||0),balance);if(amt>=0.5){toast(`Opening Stripe for ${money(amt)}…`);await startSplitStripeCheckout(amt);return}}if(balance>0.005){continueSplitWorkflow(order,`Remaining balance ${money(balance)}. Add the next tender and tap Complete Split Payment.`);toast(`Tender recorded — ${money(balance)} still due`);return}await completeSplitWorkflow(order)}
async function startSplitStripeCheckout(amount){if(!pendingPaymentOrder?.id)throw new Error("Choose an order first.");ensureSplitSession();sessionStorage.setItem("bloom_split_stripe_return","1");saveSplitSession();const description=$("#paymentDescription")?.value?.trim()||`${pendingPaymentOrder.order_number||"Order"} split card payment`;setPaymentProcessing(true);try{const d=await api("create-checkout",{method:"POST",body:JSON.stringify({amount,description,order_id:pendingPaymentOrder.id,order_number:pendingPaymentOrder.order_number||""})});if(!d.url)throw new Error("Stripe did not return a checkout link.");location.assign(d.url)}catch(e){sessionStorage.removeItem("bloom_split_stripe_return");setPaymentProcessing(false);throw e}}
async function handleSplitAfterStripe(order){loadSplitSessionFromStorage();await Promise.all([loadOrders(),loadDashboard(),loadInvoices()]);await handleSplitPaymentContinuation({order},[])}
function showPaymentSuccessPanel({headline,detail,order}){if(order)setPendingPaymentOrder(order);renderPaymentCenterShell();if($("#paymentSuccessDetail"))$("#paymentSuccessDetail").innerHTML=detail||headline||"Payment recorded.";setPaymentCenterMode("success");refreshPaymentHistory()}
async function refreshPaymentHistory(){const body=$("#paymentHistoryBody");if(!body||!pendingPaymentOrder?.id){if(body)body.innerHTML='<p class="subtle">Select an order to view payment history.</p>';return}body.innerHTML='<p class="subtle">Loading…</p>';try{const items=(await api(`payments?order_id=${encodeURIComponent(pendingPaymentOrder.id)}`)).items||[];if(!items.length){body.innerHTML='<p class="subtle">No payments recorded yet.</p>';return}body.innerHTML=`<table class="payment-history-table"><thead><tr><th>Date</th><th>Method</th><th>Amount</th><th>Status</th></tr></thead><tbody>${items.map(p=>{const label=p.metadata?.tender_label||p.method;const dt=p.received_at?new Date(p.received_at).toLocaleString():"—";return `<tr><td>${esc(dt)}</td><td>${esc(label)}</td><td>${money(p.amount)}</td><td>${esc(p.status||"SUCCEEDED")}</td></tr>`}).join("")}</tbody></table>`}catch(e){body.innerHTML=`<p class="subtle">${esc(e.message||"Could not load history.")}</p>`}}
function setPaymentProcessing(on){paymentCenterProcessing=on;["completeCashPayment","completeCheckPayment","completeSplitPayment","checkout","confirmPayLater"].forEach(id=>{const b=$("#"+id);if(b)b.disabled=on})}
async function postManualPayment(method,amount,note){if(paymentCenterProcessing)return;if(!pendingPaymentOrder?.id)throw new Error("Choose an order before recording this payment.");if(amount<=0)throw new Error("Enter a valid payment amount.");setPaymentProcessing(true);if($("#paymentStatus"))$("#paymentStatus").textContent=`Recording ${method} payment…`;try{const key=(crypto.randomUUID?crypto.randomUUID():`${Date.now()}-${Math.random()}`);const payload={order_id:pendingPaymentOrder.id,amount,method,idempotency_key:key};if(note)payload.note=note;const result=await api("payments",{method:"POST",body:JSON.stringify(payload)});return result.order||result}finally{setPaymentProcessing(false)}}
async function recordLocalPayment(method){try{const amount=Number($("#paymentAmount")?.value||0);const order=await postManualPayment(method,amount);setPendingPaymentOrder(order);const balance=Number(order?.balance_due??0);if($("#paymentStatus"))$("#paymentStatus").textContent=`${method} payment recorded. Remaining balance: ${money(balance)}.`;toast(`${method} payment recorded`);await Promise.all([loadOrders(),loadDashboard(),loadInvoices()]);return order}catch(e){const msg=e?.message||"Payment could not be recorded.";toast(msg);if($("#paymentStatus"))$("#paymentStatus").textContent=msg;throw e}}
const SPLIT_METHODS=["Cash","Check","Card","Other"];
async function submitSplitPayment(){if(paymentCenterProcessing||!pendingPaymentOrder?.id)return toast("Choose an order first");const parts=readSplitRows().filter(p=>Number(p.amount)>0);if(!parts.length)return toast("Add at least one payment part");const session=ensureSplitSession();const batchId=session.batchId;let seq=session.partSeq||0;setPaymentProcessing(true);try{$("#splitPaymentError").textContent="";if($("#paymentStatus"))$("#paymentStatus").textContent="Applying split payment…";const result=await api("payments",{method:"POST",body:JSON.stringify({action:"split",order_id:pendingPaymentOrder.id,split_batch_id:batchId,parts:parts.map(p=>{const key=`${batchId}:part:${seq++}`;return{...p,idempotency_key:key}})})});session.partSeq=seq;saveSplitSession();await handleSplitPaymentContinuation(result,result.deferred_card_parts||[])}catch(err){const msg=err?.message||"Split payment failed";$("#splitPaymentError").textContent=msg;if($("#paymentStatus"))$("#paymentStatus").textContent=msg;toast(msg)}finally{setPaymentProcessing(false)}}
function renderSplitPaymentRows(rows){const host=$("#splitPaymentRows");if(!host)return;host.innerHTML=rows.map((r,i)=>`<li class="split-part-row" data-split-index="${i}"><span class="part-num">${i+1}.</span><select data-split-method aria-label="Payment method ${i+1}">${SPLIT_METHODS.map(m=>`<option ${m===r.method?"selected":""}>${m}</option>`).join("")}</select><input type="number" min="0.01" step="0.01" data-split-amount value="${Number(r.amount||0).toFixed(2)}" aria-label="Amount ${i+1}"><input data-split-note value="${esc(r.note||"")}" placeholder="Note" aria-label="Note ${i+1}">${rows.length>1?`<button type="button" class="secondary" data-split-remove="${i}">Remove</button>`:""}</li>`).join("");updateSplitTotals()}
...
addPastelPageFrames();
// Pre-launch QA: this bell had no click handler at all — a real, visible,
// silently-broken control (tapping it did nothing, no feedback). Give it
// an honest state instead of a silent no-op until a real app-wide
// notification center exists.
$("#atelierMobileNotify")?.addEventListener("click",()=>toast("Notifications aren't set up yet — coming soon."));
...
$("#tileEditForm")?.addEventListener("submit",e=>{e.preventDefault();const f=e.currentTarget,d=Object.fromEntries(new FormData(f)),id=d.tile_id||`tile-${Date.now()}`,tile={id,name:d.name.trim(),label:"",category:d.category,image:d.image};const i=posTiles.findIndex(x=>x.id===id);if(i>=0)posTiles[i]=tile;else posTiles.push(tile);$("#tileEditDialog").close();
  // Pre-launch QA: this button is labeled "Save tile," but used to only
  // update in-memory state — the edit was silently lost if the outer tile
  // -manager dialog was then closed/canceled instead of its own separate
  // "Save tiles" button. Persist immediately so the label is accurate.
  savePosTiles();
});
...
document.addEventListener("click",e=>{let t;if(t=e.target.closest("[data-cart-minus]")){const i=Number(t.dataset.cartMinus);posCart[i].quantity=Math.max(1,Number(posCart[i].quantity||1)-1);savePosCart();return}if(t=e.target.closest("[data-cart-plus]")){const i=Number(t.dataset.cartPlus);posCart[i].quantity=Number(posCart[i].quantity||1)+1;savePosCart();return}if(t=e.target.closest("[data-cart-remove]")){posCart.splice(Number(t.dataset.cartRemove),1);savePosCart();return}if(t=e.target.closest("[data-cart-checkout]")){e.preventDefault();checkoutPosCart();return}});
document.addEventListener("change",e=>{const input=e.target.closest("[data-cart-price]");if(input){posCart[Number(input.dataset.cartPrice)].price=Number(input.value||0);savePosCart()}});
document.addEventListener("click",e=>{let t;if(t=e.target.closest("[data-load-quote]")){loadSavedQuote(t.dataset.loadQuote);return}if(t=e.target.closest("[data-delete-quote]")){if(!confirm("Delete this saved quote?"))return;savedQuotes=savedQuotes.filter(q=>q.id!==t.dataset.deleteQuote);persistSavedQuotes();return}});
...
$("#scanInventoryBannerBtn")?.addEventListener("click",openInventoryScanner);
...
function updateOrderBuilder(){const f=$("#orderForm");if(!f)return;const subtotal=orderNumber(f,"subtotal"),labor=orderNumber(f,"labor_charge"),addons=orderNumber(f,"addon_total"),delivery=orderNumber(f,"delivery_fee"),discount=orderNumber(f,"discount"),taxRate=orderNumber(f,"tax_rate"),cost=orderNumber(f,"estimated_cost");const taxable=Math.max(0,subtotal+labor+addons-discount);const tax=Math.round(taxable*(taxRate/100)*100)/100;if(f.elements.tax)f.elements.tax.value=tax.toFixed(2);const total=Math.max(0,taxable+delivery+tax),profit=total-cost;$("#orderTaxAmount").textContent=money(tax);$("#orderLiveTotal").textContent=money(total);$("#orderSummaryTotal").textContent=money(total);$("#orderSummaryProfit").textContent=money(profit);$("#orderProfitPreview").textContent=`Estimated profit ${money(profit)}`;const product=f.elements.product_id.selectedOptions[0];$("#orderSuggestion").textContent=product?.value?`${product.textContent} selected. Review recipe cost and add delivery or add-ons before saving.`:"Custom order: use the description, flower preferences, and estimated material cost so the designer has a complete worksheet."}
...
// Homecoming / Prom — a special-event details fieldset on the same order
// builder dialog (reuses orders.metadata, the existing free-form JSON column
// already wired through create_order_atomic — see the wire-order intake
// above for the same pattern). Not a separate feature or table.
const HOMECOMING_PROM_OCCASIONS=new Set(["Homecoming","Prom"]);
const HOMECOMING_FIELD_KEYS=["event_date","event_school_name","event_student_name","event_product_type","event_outfit_color","event_flower_colors","event_special_notes"];
// The matching boutonniere is NOT a second order or a second customer — it
// is a linked second piece inside the same order's metadata.homecoming_prom
// (metadata.homecoming_prom.matching_boutonniere), so event date, school,
// customer, and inspiration photo are entered exactly once and shared.
const BOUTONNIERE_FIELD_KEYS=["boutonniere_recipient_name","boutonniere_match_to","boutonniere_flower_details","boutonniere_notes"];
let orderInspirationPhotoDataUrl=null;
let orderInspirationPhotoRemoved=false;
function isHomecomingPromOccasion(value){return HOMECOMING_PROM_OCCASIONS.has(String(value||"").trim())}
function toggleHomecomingSection(){const f=$("#orderForm"),section=$("#orderHomecomingSection");if(!f||!section)return;section.hidden=!isHomecomingPromOccasion(f.elements.occasion?.value);updateHomecomingPieceSummary()}
function resetOrderInspirationPhoto(){orderInspirationPhotoDataUrl=null;orderInspirationPhotoRemoved=false;const preview=$("#orderInspirationPreview"),link=$("#orderInspirationPreviewLink"),status=$("#orderInspirationStatus"),input=$("#orderInspirationPhoto"),removeBtn=$("#orderRemoveInspirationPhoto");if(preview){preview.hidden=true;preview.removeAttribute("src")}if(link)link.removeAttribute("href");if(status){status.hidden=true;status.textContent=""}if(input)input.value="";if(removeBtn)removeBtn.hidden=true}
function showOrderInspirationPreview(url){const preview=$("#orderInspirationPreview"),link=$("#orderInspirationPreviewLink"),removeBtn=$("#orderRemoveInspirationPhoto");if(!url)return;if(preview){preview.src=url;preview.hidden=false}if(link)link.href=url;if(removeBtn)removeBtn.hidden=false}
async function loadOrderInspirationPhoto(order){resetOrderInspirationPhoto();const path=order?.metadata?.homecoming_prom?.inspiration_photo_path;if(!path||!order?.id)return;const status=$("#orderInspirationStatus"),removeBtn=$("#orderRemoveInspirationPhoto");try{const result=await api(`orders?order_id=${encodeURIComponent(order.id)}&view=inspiration_photo`);if(result.signed_url)showOrderInspirationPreview(result.signed_url);else if(status){status.hidden=false;status.textContent="Inspiration photo is on file, but the preview could not be loaded. It is still attached to this order.";if(removeBtn)removeBtn.hidden=false}}catch{if(status){status.hidden=false;status.textContent="Inspiration photo is on file, but the preview could not be loaded right now."}if(removeBtn)removeBtn.hidden=false}}
function updateHomecomingPieceSummary(){const f=$("#orderForm"),summary=$("#orderHomecomingPieceSummary");if(!f||!summary)return;const needed=Boolean(f.elements.event_matching_boutonniere_needed?.checked);const mainLabel=f.elements.event_product_type?.value||"Main piece";summary.textContent=needed?`2 PIECES — ${mainLabel} · Matching Boutonniere`:`1 PIECE — ${mainLabel}`}
function toggleBoutonniereSection(){const f=$("#orderForm"),box=$("#orderBoutonniereSection");if(!f||!box)return;box.hidden=!f.elements.event_matching_boutonniere_needed?.checked;updateHomecomingPieceSummary()}
// A boutonniere doesn't need its own matching boutonniere — that combination
// doesn't make florist sense. Disable (not just hide) the checkbox when the
// main piece IS a boutonniere, and clear/collapse any boutonniere already
// entered so a stale checked state can't be resubmitted silently.
function syncBoutonniereAvailability(){const f=$("#orderForm"),chk=$("#orderMatchingBoutonniere"),hint=$("#orderMatchingBoutonniereHint");if(!f||!chk)return;const isBoutonniere=String(f.elements.event_product_type?.value||"").trim()==="Boutonniere";chk.disabled=isBoutonniere;if(hint)hint.hidden=!isBoutonniere;if(isBoutonniere&&chk.checked){chk.checked=false;toggleBoutonniereSection()}}
// Copies the current outfit/flower-color context into the boutonniere's own
// (separately editable) flower-detail field, and the student name into its
// recipient field — a one-time convenience default, not a live link. The
// florist can simplify or fully rewrite either afterward for the second piece.
function prefillBoutonniereFromMainPiece(){const f=$("#orderForm");if(!f)return;const flowerDetails=f.elements.boutonniere_flower_details;if(flowerDetails&&!flowerDetails.value.trim()){const outfit=String(f.elements.event_outfit_color?.value||"").trim();const flowers=String(f.elements.event_flower_colors?.value||"").trim();const suggestion=[flowers?`Match main piece: ${flowers}`:"",outfit?`(dress: ${outfit})`:""].filter(Boolean).join(" ");if(suggestion)flowerDetails.value=suggestion}const recipient=f.elements.boutonniere_recipient_name;if(recipient&&!recipient.value.trim()&&f.elements.event_student_name?.value.trim())recipient.value=f.elements.event_student_name.value.trim()}
function fillHomecomingFields(order){const f=$("#orderForm");if(!f)return;const details=order?.metadata?.homecoming_prom&&typeof order.metadata.homecoming_prom==="object"?order.metadata.homecoming_prom:{};for(const key of HOMECOMING_FIELD_KEYS)if(f.elements[key])f.elements[key].value=details[key]||"";if(f.elements.event_piece_ready)f.elements.event_piece_ready.checked=Boolean(details.piece_ready);const boutonniere=details.matching_boutonniere&&typeof details.matching_boutonniere==="object"?details.matching_boutonniere:null;if(f.elements.event_matching_boutonniere_needed)f.elements.event_matching_boutonniere_needed.checked=Boolean(boutonniere?.needed);for(const key of BOUTONNIERE_FIELD_KEYS)if(f.elements[key])f.elements[key].value=boutonniere?.[key]||"";if(f.elements.boutonniere_price)f.elements.boutonniere_price.value=boutonniere?.price??"";if(f.elements.boutonniere_piece_ready)f.elements.boutonniere_piece_ready.checked=Boolean(boutonniere?.piece_ready);toggleBoutonniereSection();syncBoutonniereAvailability()}
function collectHomecomingMetadata(f){const details={};for(const key of HOMECOMING_FIELD_KEYS){const value=String(f.elements[key]?.value||"").trim();if(value)details[key]=value}if(f.elements.event_piece_ready?.checked)details.piece_ready=true;if(f.elements.event_matching_boutonniere_needed?.checked){const boutonniere={needed:true};for(const key of BOUTONNIERE_FIELD_KEYS){const value=String(f.elements[key]?.value||"").trim();if(value)boutonniere[key]=value}const price=Number(f.elements.boutonniere_price?.value||0);if(price>0)boutonniere.price=price;if(f.elements.boutonniere_piece_ready?.checked)boutonniere.piece_ready=true;details.matching_boutonniere=boutonniere}return details}
...
$("#orderOccasion")?.addEventListener("change",toggleHomecomingSection);
$("#orderEventProductType")?.addEventListener("change",()=>{updateHomecomingPieceSummary();syncBoutonniereAvailability()});
$("#orderMatchingBoutonniere")?.addEventListener("change",e=>{toggleBoutonniereSection();if(e.target.checked)prefillBoutonniereFromMainPiece()});
$("#orderInspirationPhoto")?.addEventListener("change",async e=>{const file=e.target.files?.[0];if(!file)return;if(file.size>5*1024*1024)return toast("Inspiration photo must be under 5 MB");const allowed=["image/jpeg","image/png","image/webp","image/heic","image/heif"];if(file.type&&!allowed.includes(file.type))return toast("Inspiration photo must be a JPEG, PNG, WebP, or HEIC image");orderInspirationPhotoDataUrl=await new Promise((a,b)=>{const r=new FileReader();r.onload=()=>a(r.result);r.onerror=b;r.readAsDataURL(file)});orderInspirationPhotoRemoved=false;showOrderInspirationPreview(orderInspirationPhotoDataUrl);const status=$("#orderInspirationStatus");if(status){status.hidden=false;status.textContent="New photo selected — it will be saved with this order."}});
$("#orderRemoveInspirationPhoto")?.addEventListener("click",()=>{orderInspirationPhotoDataUrl=null;orderInspirationPhotoRemoved=true;const preview=$("#orderInspirationPreview"),link=$("#orderInspirationPreviewLink"),status=$("#orderInspirationStatus"),input=$("#orderInspirationPhoto"),removeBtn=$("#orderRemoveInspirationPhoto");if(preview){preview.hidden=true;preview.removeAttribute("src")}if(link)link.removeAttribute("href");if(input)input.value="";if(removeBtn)removeBtn.hidden=true;if(status){status.hidden=false;status.textContent="Photo will be removed when you save this order."}});
bindForm("#orderForm",async e=>{e.preventDefault();const f=e.currentTarget,isEdit=Boolean(f.elements.id.value);let d=normalizeOrderFormPayload(Object.fromEntries(new FormData(f)));for(const key of [...HOMECOMING_FIELD_KEYS,...BOUTONNIERE_FIELD_KEYS,"event_matching_boutonniere_needed","event_piece_ready","boutonniere_price","boutonniere_piece_ready"])delete d[key];if(isHomecomingPromOccasion(d.occasion))d.metadata={homecoming_prom:collectHomecomingMetadata(f)};if(orderInspirationPhotoDataUrl)d.inspiration_photo_data_url=orderInspirationPhotoDataUrl;else if(orderInspirationPhotoRemoved)d.remove_inspiration_photo=true;if(isEdit){if(String(d.fulfillment||"").toUpperCase()==="DELIVERY"&&!String(d.delivery_address||"").trim())return toast("Enter a delivery address for delivery orders.")}else{const clientErrors=validateOrderFormPayload(d);if(clientErrors.length)return toast(clientErrors[0])}if(d.fulfillment==="DELIVERY"&&d.delivery_address?.trim()&&!Number(d.delivery_miles||0))await calculateRoute(false);d.total_preview=Number(String($("#orderLiveTotal").textContent).replace(/[^0-9.-]/g,""));const result=await api("orders",{method:isEdit?"PATCH":"POST",body:JSON.stringify(d)});const order=result.item||{};if(!isEdit)setPendingPaymentOrder(order);if(result.inventoryAdjustments?.length)toast(`Inventory updated for ${result.inventoryAdjustments.length} recipe item${result.inventoryAdjustments.length===1?"":"s"}`);if(result.inventoryWarnings?.length)toast(`Order saved. Inventory note: ${result.inventoryWarnings[0]}`);const goToPayment=!isEdit&&d.customer_type!=="BUSINESS"&&String(d.payment_required||"YES").toUpperCase()!=="NO";f.reset();syncOrderFormUi(false);resetOrderInspirationPhoto();$("#orderDialog").close();toast(isEdit?"Order updated":goToPayment?"Order created — opening Payment Center":d.customer_type==="BUSINESS"?"Order saved — invoice this business account when ready":String(d.payment_required||"").toUpperCase()==="NO"?"Order saved — pay later when ready":"Order created");await Promise.all([loadOrders(),loadDashboard(),loadInvoices()]);if(goToPayment){await openPaymentCenterForOrder(order)}});
...
let deliveryProofDataUrl=null;
function openDeliveryProof(delivery,viewOnly=false){const f=$("#deliveryProofForm");if(!f||!delivery)return;f.reset();deliveryProofDataUrl=null;f.elements.id.value=delivery.id;f.elements.recipient_name.value=delivery.recipient_name||"";f.elements.signature_name.value=delivery.signature_name||"";f.elements.delivery_note.value=delivery.notes||"";const preview=$("#deliveryProofPreview"),photo=$("#deliveryProofPhoto"),save=$("#deliveryProofSave"),reasonWrap=$("#noPhotoReasonWrap");if(preview){preview.hidden=true;preview.removeAttribute("src")}if(photo)photo.value="";if(reasonWrap)reasonWrap.hidden=true;if(viewOnly&&delivery.proof_signed_url&&preview){preview.src=delivery.proof_signed_url;preview.hidden=false;if(save)save.hidden=true}else{if(save){save.hidden=false;save.textContent="Save proof & mark delivered"}}$("#deliveryProofDialog").showModal()}
...
$("#saveShopHours")?.addEventListener("click",saveShopHours);
HOURS_WEEKDAY_SHORT.forEach((_,weekday)=>{$(`#hoursClosed${weekday}`)?.addEventListener("change",e=>{document.querySelector(`.hours-row[data-weekday="${weekday}"]`)?.classList.toggle("hours-row-closed",e.target.checked)})});
...
$$('[data-hero-image]').forEach(b=>b.onclick=()=>{$("#heroImageUrl").value=b.dataset.heroImage;renderWebsite();toast("Website photo selected")});
$("#heroImageUpload")?.addEventListener("change",e=>{const file=e.target.files?.[0];if(!file)return;if(file.size>900*1024)return toast("Please use a photo under 900 KB");const reader=new FileReader();reader.onload=()=>{$("#heroImageUrl").value=reader.result;renderWebsite();toast("Your arrangement photo is now in the preview")};reader.readAsDataURL(file)});
$$('[data-gallery-filter]').forEach(b=>b.onclick=()=>{$$('[data-gallery-filter]').forEach(x=>x.classList.remove("active"));b.classList.add("active");const filter=b.dataset.galleryFilter;$$('.photo-gallery [data-category]').forEach(card=>card.hidden=filter!=="all"&&card.dataset.category!==filter)});
async function duplicateProduct(id){const source=products.find(x=>x.id===id);if(!source)return;const recipe=(await api(`recipes?product_id=${id}`)).items||[];const payload={...source,id:undefined,name:`${source.name} Copy`,sku:source.sku?`${source.sku}-COPY`:"",available_online:false,featured:false};const {item}=await api("products",{method:"POST",body:JSON.stringify(payload)});await api("recipes",{method:"POST",body:JSON.stringify({product_id:item.id,items:recipe.map(r=>({ingredient_name:r.ingredient_name,quantity:r.quantity,unit:r.unit,unit_cost:r.unit_cost}))})});await loadProducts();toast(`${item.name} duplicated`)}
...
// The "Daisy" mini-dock button used to carry a stale data-route/data-page
// pointing at the Dashboard — Daisy has no page of her own, she only
// exists as a persona inside the Lily-platform chat panel. Open that
// panel already switched to Daisy instead.
$("[data-assistant='daisy']")?.addEventListener("click",()=>window.BloomLilyPlatform?.open?.("daisy"));
setAssistantPersona("Lily");
$("#assistantMic")?.addEventListener("click",()=>{const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR)return toast("Voice questions are not supported in this browser. You can still type your question.");const r=new SR();r.lang="en-US";r.interimResults=false;r.maxAlternatives=1;const listening=$("#assistantListening");if(listening)listening.hidden=false;r.onresult=e=>{const input=$("#assistantPrompt");if(input)input.value=e.results[0][0].transcript;if(listening)listening.hidden=true};r.onerror=()=>{if(listening)listening.hidden=true;toast("I couldn't hear that clearly. Please try again.")};r.onend=()=>{if(listening)listening.hidden=true};r.start()});
$("#assistantRun")?.addEventListener("click",()=>runAssistant());

function aiGeneratedText(value){if(value==null)return "";if(typeof value==="string")return value.trim();if(typeof value==="number"||typeof value==="boolean")return String(value);if(Array.isArray(value)){const parts=value.map(aiGeneratedText).filter(Boolean);return parts.join(parts.length>1?"\n":"")}if(typeof value==="object"){const textKeys=["text","wording","content","result","response","description","caption","value","answer","message","reply","summary","output","body"];for(const key of textKeys){if(Object.prototype.hasOwnProperty.call(value,key)&&value[key]!=null){const found=aiGeneratedText(value[key]);if(found)return found}}const flattened=[];for(const [key,nested] of Object.entries(value)){if(nested==null||nested==="")continue;if(key==="error"||key==="status")continue;const found=aiGeneratedText(nested);if(found){flattened.push(found.includes("\n")?`${key}:\n${found}`:`${key}: ${found}`)}}return flattened.slice(0,8).join("\n")}return ""}
async function runContentHelper(button){const target=document.getElementById(button.dataset.aiTarget);if(!target)return;const form=button.closest("form"),fields=form?Object.fromEntries(new FormData(form)):{};const original=button.textContent;button.disabled=true;button.textContent="Lily is writing…";try{const kind=button.dataset.aiKind||"description",focus=$("#websiteAiMode")?.value||"polished florist marketing";const safeShop={name:shopSettings?.name||"",address:shopSettings?.address||"",phone:shopSettings?.phone||"",email:shopSettings?.email||"",website_style:shopSettings?.website_style||"",tagline:shopSettings?.tagline||""};const d=await smartAi({mode:"generate",model:selectedAiModel(),task:`write ${kind} with a ${focus} focus`,input:{current:target.value||"",fields,shop:safeShop,instructions:"Write natural, specific florist copy. For SEO, mention the shop city or delivery area and relevant floral service naturally, avoid keyword stuffing, and include a clear customer action."},schema:{text:"plain text finished wording only"}});const wording=aiGeneratedText(d?.result??d);if(!wording)throw new Error("Lily returned an empty response. Please try again.");target.value=wording;target.dispatchEvent(new Event("input",{bubbles:true}));toast("Lily added polished wording. Review it, then save.")}catch(e){toast(`AI unavailable: ${e.message}`)}finally{button.disabled=false;button.textContent=original}}
document.addEventListener("click",e=>{const b=e.target.closest("[data-ai-target]");if(b)runContentHelper(b)});
$("#librarySearch")?.addEventListener("input",renderLibrary);$("#libraryCategory")?.addEventListener("change",renderLibrary);
$("#addLibraryRecipeLine")?.addEventListener("click",()=>{$("#libraryRecipeRows").insertAdjacentHTML("beforeend",libraryRecipeRows([["",1]]))});
$("#libraryDesignForm")?.addEventListener("submit",async e=>{e.preventDefault();const b=$("#addLibraryDesignButton");b.disabled=true;try{await addLibraryProduct()}finally{b.disabled=false}});

let wakeRecognition=null,wakeEnabled=false,wakeRestartTimer=null,wakeBusy=false,wakeAwaitingPersona=null,wakeIgnoreUntil=0;
function wakeUi(message,state="") {const box=$("#assistantWakeToggle")?.parentElement,status=$("#assistantWakeStatus"),button=$("#assistantWakeToggle");if(status)status.textContent=message;if(box){box.classList.toggle("active",wakeEnabled);box.classList.toggle("listening",state==="listening");box.classList.toggle("error",state==="error")}if(button)button.textContent=wakeEnabled?'⏹ Turn off “Hey Lily / Hey Rose”':'🎙️ Turn on “Hey Lily / Hey Rose”'}
function stopWakeListening(){wakeEnabled=false;clearTimeout(wakeRestartTimer);wakeRestartTimer=null;try{wakeRecognition?.stop()}catch{}wakeRecognition=null;wakeBusy=false;wakeAwaitingPersona=null;wakeUi("Hands-free listening is off.")}
function acknowledgeWake(persona){const response=persona==="Rose"?"I’m here, sweetheart. Tell me what needs your attention.":"Absolutely—I'm right here with you, and I’m ready to help.";const out=$("#assistantAnswer");if(out)out.textContent=`${persona}: ${response}`;speakAssistant(response,persona,true)}
function handleWakeTranscript(transcript){if(Date.now()<wakeIgnoreUntil||wakeBusy)return;const clean=(transcript||"").trim();const match=clean.match(/(?:hey\s+)?(lily|rose)\b[\s,.:;-]*(.*)$/i);if(!match){if(wakeAwaitingPersona&&clean){const persona=wakeAwaitingPersona;wakeAwaitingPersona=null;const input=$("#assistantPrompt");if(input)input.value=clean;wakeBusy=true;runAssistant(clean,persona).finally(()=>{wakeBusy=false})}return}const persona=match[1].toLowerCase()==="rose"?"Rose":"Lily";const command=(match[2]||"").trim();assistantPersona=persona;setAssistantPersona(persona);wakeIgnoreUntil=Date.now()+1200;acknowledgeWake(persona);if(command){wakeAwaitingPersona=null;const input=$("#assistantPrompt");if(input)input.value=command;wakeBusy=true;setTimeout(()=>runAssistant(command,persona).finally(()=>{wakeBusy=false}),650)}else{wakeAwaitingPersona=persona;const input=$("#assistantPrompt");if(input)input.value="";wakeUi(`${persona} heard you. Say your question now…`,"listening")}}
function startWakeListening(){const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){wakeUi("Hands-free wake phrases are not supported in this browser. Use Chrome or Edge.","error");return}wakeEnabled=true;wakeRecognition=new SR();wakeRecognition.lang="en-US";wakeRecognition.continuous=true;wakeRecognition.interimResults=false;wakeRecognition.maxAlternatives=1;wakeRecognition.onstart=()=>wakeUi('Listening for “Hey Lily” or “Hey Rose”…',"listening");wakeRecognition.onresult=e=>{for(let i=e.resultIndex;i<e.results.length;i++){if(e.results[i].isFinal)handleWakeTranscript(e.results[i][0].transcript)}};wakeRecognition.onerror=e=>{if(e.error==="not-allowed"||e.error==="service-not-allowed"){wakeEnabled=false;wakeUi("Microphone permission was blocked. Allow microphone access, then turn this on again.","error")}else if(e.error!=="no-speech"&&e.error!=="aborted")wakeUi(`Voice listening paused: ${e.error}. Retrying…`,"error")};wakeRecognition.onend=()=>{wakeRecognition=null;if(wakeEnabled){clearTimeout(wakeRestartTimer);wakeRestartTimer=setTimeout(()=>{if(wakeEnabled&&!wakeBusy)startWakeListening()},500)}};try{wakeRecognition.start()}catch(e){wakeUi("Could not start microphone listening. Refresh and try again.","error")}}
$("#assistantWakeToggle")?.addEventListener("click",()=>wakeEnabled?stopWakeListening():startWakeListening());
document.addEventListener("visibilitychange",()=>{if(document.hidden&&wakeRecognition){try{wakeRecognition.stop()}catch{}}else if(!document.hidden&&wakeEnabled&&!wakeRecognition)startWakeListening()});
document.addEventListener("click",async e=>{let t;
if(t=e.target.closest("[data-view-customer]")){const c=customers.find(x=>x.id===t.dataset.viewCustomer);return c&&window.BloomCustomerProfile?.openProfile?.(c,{orders,session,showPage})}
...
if(t=e.target.closest("[data-inventory-color]")){$("#inventoryColorFilter").value=t.dataset.inventoryColor;renderInventory();$$(`[data-inventory-color]`).forEach(x=>x.classList.toggle("active",x===t));return}
...
if(t=e.target.closest("[data-edit-expense]"))return openExpense(expenses.find(x=>x.id===t.dataset.editExpense));
if(t=e.target.closest("[data-delete-expense]")){if(confirm("Delete this expense? This cannot be undone.")){try{await api("expenses",{method:"DELETE",body:JSON.stringify({id:t.dataset.deleteExpense})});await Promise.all([loadExpenses(),loadDashboard()]);toast("Expense deleted")}catch(err){toast(err.message)}}return}
if(t=e.target.closest("[data-preview-library]"))return openLibraryDesign(t.dataset.previewLibrary);
if(t=e.target.closest("[data-library-card]")){if(!e.target.closest("button"))return openLibraryDesign(t.dataset.libraryCard)}
if(t=e.target.closest("[data-remove-library-line]")){t.closest(".library-recipe-row")?.remove();return}
if(t=e.target.closest("[data-duplicate-product]")){t.disabled=true;try{await duplicateProduct(t.dataset.duplicateProduct)}finally{t.disabled=false}return}
if(t=e.target.closest("[data-publish-product]")){t.disabled=true;try{await toggleProductPublish(t.dataset.publishProduct)}finally{t.disabled=false}return}
...
$("#printManifest").onclick=()=>{document.body.classList.add("manifest-print");window.print();setTimeout(()=>document.body.classList.remove("manifest-print"),500)};
$("#closeReceipt").onclick=()=>$("#receiptDialog").close();$("#printReceipt").onclick=()=>printReceiptOnly();
document.querySelector("#paymentsPage")?.addEventListener("click",e=>{const modeBtn=e.target.closest("[data-payment-mode]");if(modeBtn){const m=modeBtn.dataset.paymentMode;setPaymentCenterMode(m==="pay-later"?"pay-later":m);return}const moreBtn=e.target.closest("[data-payment-more]");if(moreBtn){const method=moreBtn.dataset.paymentMore;const btn=$("#completeCheckPayment");if(btn)btn.dataset.moreMethod=method;const h2=$("#paymentFlowCheck")?.querySelector("h2");if(h2)h2.textContent=`${method} payment`;setPaymentCenterMode("check");return}const quick=e.target.closest("[data-cash-quick]");if(quick){applyCashQuick(quick.dataset.cashQuick);return}});
$("#paymentCenterBackOrder")?.addEventListener("click",returnFromPaymentCenter);
$("#paymentCenterCancel")?.addEventListener("click",cancelPaymentCenter);
$("#cashPaymentAmount")?.addEventListener("input",updateCashMath);
$("#cashReceived")?.addEventListener("input",updateCashMath);
$("#completeCashPayment")?.addEventListener("click",completeCashPaymentFlow);
$("#completeCheckPayment")?.addEventListener("click",completeCheckPaymentFlow);
$("#completeSplitPayment")?.addEventListener("click",submitSplitPayment);
$("#confirmPayLater")?.addEventListener("click",returnFromPaymentCenter);
$("#paymentSuccessPrint")?.addEventListener("click",()=>{if(pendingPaymentOrder)openReceiptWithPayments(pendingPaymentOrder)});
$("#paymentSuccessReturn")?.addEventListener("click",returnFromPaymentCenter);
function applyCashQuick(kind){const balance=getPaymentBalance();if(kind==="exact"){$("#cashPaymentAmount").value=balance.toFixed(2);$("#cashReceived").value=balance.toFixed(2)}else{$("#cashReceived").value=Number(kind).toFixed(2)}updateCashMath()}
async function completeCashPaymentFlow(){try{const amount=Number($("#cashPaymentAmount")?.value||0),received=Number($("#cashReceived")?.value||0);const order=await postManualPayment("Cash",amount);await Promise.all([loadOrders(),loadDashboard(),loadInvoices()]);const change=Math.max(0,Math.round((received-amount)*100)/100);showPaymentSuccessPanel({detail:`Payment received: ${money(amount)}<br>Change due: ${money(change)}<br>Remaining balance: ${money(order.balance_due||0)}`,order});toast("Cash payment recorded")}catch(e){toast(e.message)}}
async function completeCheckPaymentFlow(){try{const method=$("#completeCheckPayment")?.dataset.moreMethod||"Check";delete $("#completeCheckPayment").dataset.moreMethod;const amount=Number($("#checkPaymentAmount")?.value||0),note=$("#checkPaymentNote")?.value||"";const order=await postManualPayment(method,amount,note);await Promise.all([loadOrders(),loadDashboard(),loadInvoices()]);showPaymentSuccessPanel({detail:`${method} payment ${money(amount)} recorded.<br>Remaining balance: ${money(order.balance_due||0)}`,order});toast(`${method} payment recorded`)}catch(e){toast(e.message)}}
$("#addSplitPaymentRow")?.addEventListener("click",()=>{const rows=readSplitRows();rows.push({method:"Cash",amount:0,note:""});renderSplitPaymentRows(rows)});
$("#splitPaymentRows")?.addEventListener("input",updateSplitTotals);
$("#splitPaymentRows")?.addEventListener("click",e=>{const rm=e.target.closest("[data-split-remove]");if(!rm)return;const rows=readSplitRows();rows.splice(Number(rm.dataset.splitRemove),1);renderSplitPaymentRows(rows.length?rows:[{method:"Cash",amount:0,note:""}])});
$("#useFullBalance")?.addEventListener("click",()=>{const balance=getPaymentBalance();if(balance>0){$("#cashPaymentAmount").value=balance.toFixed(2);$("#cashReceived").value=balance.toFixed(2);updateCashMath()}});
$("#clearPendingPayment")?.addEventListener("click",()=>{setPendingPaymentOrder(null);paymentReturnOrderId=null;renderPaymentCenterShell()});
async function applyPaymentHubCheckout(){const statusEl=$("#paymentStatus");if(!window.BloomPaymentHub?.applyCheckoutMethods)return;window.BloomPaymentHub.api=api;try{await window.BloomPaymentHub.applyCheckoutMethods()}catch(e){const msg=e?.message||"Could not load checkout payment methods.";if(statusEl)statusEl.textContent=msg}}
async function sendOrderPaymentLink(mode){if(!pendingPaymentOrder?.id)return toast("Choose an order before sending a payment link");const amount=Number($("#paymentAmount")?.value||0),status=$("#paymentStatus");try{status.textContent="Creating secure link…";const d=await window.BloomPaymentHub.createPaymentLink(pendingPaymentOrder.id,{amount:amount>0?amount:undefined,email:mode==="email",sms:mode==="sms",baseUrl:location.origin});if(mode==="copy"||!mode){await navigator.clipboard.writeText(d.url);toast("Payment link copied")}else toast(mode==="email"?"Email payment link logged":"SMS payment link ready");const qr=$("#paymentLinkQr");if(qr&&d.url&&window.BloomPaymentLink?.renderQr)qr.innerHTML=`<p>QR for customer:</p>${window.BloomPaymentLink.renderQr(d.url)}`;status.textContent=`Link active — ${d.url}`}catch(e){toast(e.message);status.textContent=e.message}}
...
function readBrandImage(file,target,max=1500000){if(!file)return;if(file.size>max){toast(`Please choose an image smaller than ${(max/1000000).toFixed(1)} MB.`);return}const reader=new FileReader();reader.onload=()=>{const el=$(target);if(el)el.value=reader.result;if(target==="#websiteLogoUrl")renderWebsite();else previewBrandingForm()};reader.readAsDataURL(file)}
$("#websiteLogoUpload")?.addEventListener("change",e=>readBrandImage(e.target.files?.[0],"#websiteLogoUrl"));
$("#removeWebsiteLogo")?.addEventListener("click",()=>{$("#websiteLogoUrl").value="";$("#websiteLogoUpload").value="";renderWebsite();const p=$("#websiteLogoPreview");if(p)p.hidden=true;const h=$("#websiteLogoPlaceholder");if(h)h.hidden=false});
...
$("#aiModelSelect")?.addEventListener("change",e=>{localStorage.setItem("bloomAiModel",e.target.value);toast("Local AI model saved on this computer")});
$("#installAiModel")?.addEventListener("click",async()=>{const b=$("#installAiModel"),model=$("#aiModelSelect")?.value||selectedAiModel();b.disabled=true;b.textContent="Installing… this can take several minutes";try{await localAi("/models/install",{method:"POST",body:JSON.stringify({model})});toast(`${model} installed`);await refreshAiStatus()}catch(e){toast(e.message)}finally{b.disabled=false;b.textContent="Install selected model"}});
$("#testLocalAi")?.addEventListener("click",async()=>{const b=$("#testLocalAi");b.disabled=true;b.textContent="Testing…";try{const d=await localAi("/chat",{method:"POST",body:JSON.stringify({model:selectedAiModel(),persona:"Lily",prompt:"Reply with one short sentence confirming Florisyn Local AI is ready.",context:{}})});$("#aiDiagnosticMessage").textContent=aiGeneratedText(d?.answer??d?.message??d?.result??d)||"The local AI bridge replied, but it did not return usable text.";toast("Local AI test passed")}catch(e){toast(e.message)}finally{b.disabled=false;b.textContent="Test Lily"}});
document.querySelector('[data-page="settingsPage"]')?.addEventListener("click",()=>setTimeout(refreshAiStatus,100));
document.querySelectorAll("#settingsPage [data-settings-tab]").forEach(btn=>{btn.addEventListener("click",()=>{const tab=btn.getAttribute("data-settings-tab");document.querySelectorAll("#settingsPage [data-settings-tab]").forEach(b=>{b.classList.toggle("active",b===btn);b.setAttribute("aria-selected",String(b===btn))});document.querySelectorAll("#settingsPage [data-settings-tab-panel]").forEach(p=>{p.hidden=p.getAttribute("data-settings-tab-panel")!==tab})})});


/* Bloom v20 — low-cost AI router + Florisyn Photo Studio */
...
$("#shotEnhance")?.addEventListener("click",async()=>{if(!shotImage)return toast("Upload a photo first");const btn=$("#shotEnhance"),out=$("#shotEnhanceOut"),orig=btn.textContent;btn.disabled=true;btn.textContent="Lily is studying the photo…";try{const image_base64=shotImageForVision();const d=await api("photo-studio-ai",{method:"POST",body:JSON.stringify({action:"enhance-suggest",image_base64})});if(!d.ok){if(out){out.hidden=false;out.innerHTML=`<p class="subtle">${esc(d.message||d.error||"Lily couldn't assess this photo.")}</p>`}return}const labels={clean:"Clean & bright",luxury:"Luxury",warm:"Warm shop",true:"True color"};if(d.recommendation){const{preset,reason}=d.recommendation;if(out){out.hidden=false;out.innerHTML=`<p>Lily suggests <strong>${esc(labels[preset]||preset)}</strong>.</p><p class="enhance-reason">${esc(reason)}</p><button type="button" class="primary" id="shotEnhanceApply">Apply ${esc(labels[preset]||preset)}</button>`;$("#shotEnhanceApply").onclick=()=>{setShotPreset(preset);toast(`Applied ${labels[preset]||preset}`)}}}else if(out){out.hidden=false;out.innerHTML=`<p class="subtle">${esc(d.message||d.assessment||"Lily looked at the photo.")}</p>`}}catch(e){toast(e.message)}finally{btn.disabled=false;btn.textContent=orig}});
$("#shotGenerateImage")?.addEventListener("click",async()=>{if(!shotImage)return toast("Upload a photo first");const btn=$("#shotGenerateImage"),out=$("#shotGenerateImageOut"),orig=btn.textContent;btn.disabled=true;btn.textContent="Lily is generating…";if($("#shotStatus"))$("#shotStatus").textContent="Lily is generating a new marketing image…";try{const style=$("#shotGenStyle")?.value||"website_hero";const flowers=(shotRecipe||[]).map(r=>r.ingredient_name).filter(Boolean);const image_base64=flowers.length?undefined:shotImageForVision();const d=await api("photo-studio-ai",{method:"POST",body:JSON.stringify({action:"generate-image",style,flowers,image_base64,occasion:$("#shotOccasion")?.value||"",shop_name:shopSettings?.name||""})});if(!d.ok){toast(d.error||"Image generation failed");return}if(out){out.hidden=false;out.innerHTML=`<p class="generate-label">AI-generated — not an edit of your photo</p><img src="${esc(d.url)}" alt="AI-generated marketing image"><br><a class="secondary" href="${esc(d.url)}" download target="_blank" rel="noopener">Download</a>`}toast("New image generated")}catch(e){toast(e.message)}finally{btn.disabled=false;btn.textContent=orig;if($("#shotStatus"))$("#shotStatus").textContent=""}});
...
function applyRemoteAdminConfig(config={}){
  ...
  if(config.account_status==="maintenance"||config.account_status==="suspended"){
    ...
    lock.innerHTML=`<div><span>${suspended?"🔒":"🛠️"}</span><h2>${suspended?"Account temporarily unavailable":"Florisyn maintenance in progress"}</h2><p>${esc(config.support_message||"Please contact Florisyn Support for assistance.")}</p><button type="button" onclick="location.reload()">Check again</button></div>`;
    lock.hidden=false;
  }
}
...
function lilyVoice(text){
  if(window.FlorisynAssistantVoice?.speak){window.FlorisynAssistantVoice.speak("Lily",text,{force:true,respectToggle:false});return}
  const clean=aiGeneratedText(text);
  if(!aiStudioVoiceEnabled||!window.speechSynthesis||!clean)return;
...
function aiStudioTasks(items=[],active=-1){
  ...
  q.innerHTML=items.map((x,i)=>`<div class="ai-task-row ${i<active?"done":""}"><span>${i<active?"✓":i===active?"⏳":"○"}</span><span>${x}</span></div>`).join("");
}
function normalizedStudioDraft(raw={}){
...
function renderAiStudioDraft(){
  ...
  for(const [k,v] of Object.entries(aiStudioDraft.product||{}))if(v&&typeof v!="object")cards.push(`<article class="ai-change-card"><small>Product · ${k.replaceAll("_"," ")}</small><p>${escapeHtml(aiGeneratedText(v))}</p></article>`);
...
$("#aiStudioApply")?.addEventListener("click",()=>{
  ...
  let applied=0;
  for(const [key,id] of Object.entries(aiStudioFieldMap)){const value=aiStudioDraft.website?.[key],el=document.getElementById(id);if(value&&el){el.value=value;el.dispatchEvent(new Event("input",{bubbles:true}));applied++}}
  const product=aiStudioDraft.product||{};
  if(Object.keys(product).length){const form=$("#productForm");if(form){if(product.name)form.elements.name.value=product.name;if(product.category)form.elements.category.value=product.category;if(product.price!==undefined)form.elements.price.value=product.price;if(product.description)form.elements.description.value=product.description;if(product.seo_title)form.elements.seo_title.value=product.seo_title;if(product.seo_description)form.elements.seo_description.value=product.seo_description;$("#productDialogTitle").textContent="Review Lily's product draft";$("#productDialog")?.showModal();applied++}}
