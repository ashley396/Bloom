...
function renderPosTiles(){const grid=$("#productPadGrid");if(!grid)return;if(!Array.isArray(posTiles)||!posTiles.length){try{posTiles=structuredClone(DEFAULT_POS_TILES)}catch{posTiles=[]}}renderPosCatTabs();const filter=$("#tileCategoryFilter")?.value||"all";const q=($("#posProductSearch")?.value||"").trim().toLowerCase();let visible=(!filter||filter==="all")?posTiles:posTiles.filter(tile=>tile.category===filter);if(q)visible=visible.filter(tile=>tile.name.toLowerCase().includes(q));const customTile=`<button class="pad pad-custom quick-sale-pad" type="button" data-sale-item="Custom item" aria-label="Custom item — enter your own price"><span class="pad-add-plus">＋</span>Custom item</button>`;grid.innerHTML=customTile+visible.map(tile=>`<button class="pad quick-sale-pad" type="button" style="--cat:${posTileColor(tile.category)}" data-tile-id="${esc(tile.id)}" data-sale-item="${esc(tile.name)}" aria-label="${esc(tile.name)}">${tile.image?`<img class="pad-photo" src="${esc(tile.image)}" alt="" loading="lazy">`:""}<span class="pad-name">${esc(tile.name)}</span></button>`).join("")+`<button class="pad pad-add" id="addTileFromGrid" type="button"><span class="pad-add-plus">＋</span>Add tile</button>`}
