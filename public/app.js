...
function resetOrderInspirationPhoto(){orderInspirationPhotoDataUrl=null;const preview=$("#orderInspirationPreview"),link=$("#orderInspirationPreviewLink"),status=$("#orderInspirationStatus"),input=$("#orderInspirationPhoto");if(preview){preview.hidden=true;preview.removeAttribute("src")}if(link)link.removeAttribute("href");if(status){status.hidden=true;status.textContent=""}if(input)input.value=""}
function showOrderInspirationPreview(url){const preview=$("#orderInspirationPreview"),link=$("#orderInspirationPreviewLink");if(!url)return;if(preview){preview.src=url;preview.hidden=false}if(link)link.href=url}
async function loadOrderInspirationPhoto(order){resetOrderInspirationPhoto();const path=order?.metadata?.homecoming_prom?.inspiration_photo_path;if(!path||!order?.id)return;const status=$("#orderInspirationStatus");try{const result=await api(`orders?order_id=${encodeURIComponent(order.id)}&view=inspiration_photo`);if(result.signed_url)showOrderInspirationPreview(result.signed_url);else if(status){status.hidden=false;status.textContent="Inspiration photo is on file, but the preview could not be loaded. It is still attached to this order."}}catch{if(status){status.hidden=false;status.textContent="Inspiration photo is on file, but the preview could not be loaded right now."}}}
