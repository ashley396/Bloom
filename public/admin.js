...
function showPasswordLogin(){
  ...
  if($('#adminMfaForm'))$('#adminMfaForm').hidden=true;
  if($('#adminMfaEnrollBlock'))$('#adminMfaEnrollBlock').hidden=true;
...
function showAdminMfaUi({mode,qrCode='',secret=''}={}){
  ...
  if(mode==='enroll'){
    enroll.hidden=false;
    if(intro)intro.textContent='Install a free authenticator app on your phone, scan the QR code, then enter the 6-digit code below.';
    const qr=$('#adminMfaQr');
...
function renderAudit(){const a=selectedData?.audit||[];$('#auditList').innerHTML=a.length?a.map(x=>`<div class="audit-item"><span>${new Date(x.created_at).toLocaleString()}</span><strong>${escapeHtml(String(x.action||'').replaceAll('_',' '))}</strong><code>${escapeHtml(JSON.stringify(x.details))}</code></div>`).join(''):'<p>No admin changes recorded for this account.</p>'}
