...
function friendlyAdminError(message){
  ...
  if(/Unexpected Florisyn error/i.test(detail))return'Florisyn Administration could not finish loading. Please refresh, sign in again, or contact support if it continues.';
  if(/Please sign in again|jwt|token|expired|unauthorized/i.test(detail))return'Your admin session expired. Please sign in again.';
  return detail||'Could not complete that admin request. Please try again.';
}
...
async function initializeAdmin(){
  try{
    ...
    if(session){
      call('admin-command-center?action=dashboard').then(showApp).catch((err)=>{localStorage.removeItem('bloom_admin_session');session=null;$('#adminAuth').hidden=false;if($('#loginMessage'))$('#loginMessage').textContent=friendlyAdminError(err.message)});
    }else $('#adminAuth').hidden=false;
  }catch(err){$('#adminAuth').hidden=false;$('#loginMessage').textContent=friendlyAdminError(err.message)}
}
