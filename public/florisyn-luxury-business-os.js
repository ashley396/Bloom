...
(function () {
  ...
  // no persistence at all, so the confirmation toast claimed something was
  // saved when it vanished the instant the florist left this tab or
  // refreshed. This is a personal, per-device reminder list (not shared
  // business data), so localStorage — scoped per shop, same key convention
  // as bloom_active_shop_id — is a real fix here without a new backend
  // table/migration.
