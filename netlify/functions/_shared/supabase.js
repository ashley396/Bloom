...
export async function currentUser(event){
 ...
 let shopId=profile?.default_shop_id;
 if(!shopId){const {data:m}=await client.from("shop_members").select("shop_id").eq("user_id",data.user.id).limit(1).maybeSingle();shopId=m?.shop_id}
 if(!shopId)throw new Error("No Bloom shop is connected to this account.");
 return{client,user:data.user,shopId}
}
