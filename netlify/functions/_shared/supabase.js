...
export function fail(error){structuredLog("error","function_error",{message:error.message,status:error.statusCode||500});console.error("Bloom function error:",error);return{statusCode:error.statusCode||500,headers:{"Content-Type":"application/json","Cache-Control":"no-store"},body:JSON.stringify({error:safePublicError(error)})}}
