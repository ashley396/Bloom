...
export async function handler(event){
  ...
  try{
    ...
    const response=await fetch(`${url}/auth/v1/signup?redirect_to=${encodeURIComponent(`${siteUrl}/login.html?confirmed=1`)}`,{
      ...
      body:JSON.stringify({email:body.email,password:body.password,data:{full_name:body.fullName||"",shop_name:body.shopName||"My Flower Shop",business_phone:body.businessPhone||"",business_type:body.businessType||"",business_address:body.businessAddress||"",business_city:body.businessCity||"",business_state:body.businessState||"",business_zip:body.businessZip||"",plan_code:["starter","professional","premium"].includes(body.planCode)?body.planCode:"professional",subscription_price:[59,99,179].includes(Number(body.subscriptionPrice))?Number(body.subscriptionPrice):99,trial_days:14,trial_started_at:new Date().toISOString()}})
    });
