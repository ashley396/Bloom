...
export async function handler(event){
 ...
 try{
  ...
  const [orders,inventory,customers,expenses,payments]=await Promise.all([
  ...
  const allOrders=orders.data||[],stock=inventory.data||[],allExpenses=expenses.data||[],ledger=payments.error?null:(payments.data||[]),today=new Date().toISOString().slice(0,10);
