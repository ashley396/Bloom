...
export async function handler(event){
  ...
  try{
    ...
    const [{data:shop},{data:inventory},{data:orders},{data:deliveries}]=await Promise.all([
      ...
      client.from("inventory_items").select("name,color,variety,quantity,unit,low_stock_level,cost,price,arrival_date,vase_life_days").eq("shop_id",shopId).order("updated_at",{ascending:false}).limit(30),
    ...
    return json(200,{context:{shop:shop||{},inventory:inventory||[],recent_orders:orders||[],deliveries:deliveries||[]}});
