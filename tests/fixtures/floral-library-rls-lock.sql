...
-- Test-only copy: the original executable migration
-- (supabase/migrations/20260801_p0_01_floral_library_schema_lock_v1.sql) was
-- folded into the canonical greenfield baseline and removed from the
-- production migration chain (commit 5546ab5, "Remove archived migrations
-- from executable chain") — its cumulative effect is already present when a
-- fresh database is built from the baseline. This test harness instead
-- bootstraps a deliberately *insecure* pre-lock schema
-- (tests/fixtures/floral-library-rls-bootstrap.sql, "Reproduce the insecure
-- prior policy that existed before P0-01") specifically so it can verify
-- this lock's insecure->secure transition, which the baseline can't
-- exercise since it's never in the insecure state to begin with. Restored
-- verbatim from git history (commit 6da727e) — content unchanged.
--
