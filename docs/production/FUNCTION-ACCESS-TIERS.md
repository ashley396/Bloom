...
| `marketing-studio` | `super_admin` only — `platformAdmin(event, ["super_admin"])`, additionally gated behind the `MARKETING_STUDIO` feature flag (default off; set via `FLORISYN_FLAG_MARKETING_STUDIO`) | Brand Brain update/forget/reset, `plan_month`, `approve_content`, `generate_content`, `upload_clone_reference_photo`, `request_clone_enrollment`, `preview_clone_profile`, `revoke_clone_consent`, `set_content_disclosure`, `enqueue_publish`, `run_publishing_queue`, `connect_platform`, `disconnect_platform`, `create_ab_experiment`, `evaluate_ab_experiment`, and the Personal Brand Studio actions (`update_personal_brand_profile`, `update_personal_brand_preferences`, `forget_personal_brand_trait`, `reset_personal_brand_preferences`, `record_personal_brand_signal`, `upload_personal_brand_reference_photo`, `update_personal_brand_reference_photo`, `delete_personal_brand_reference_photo`, `submit_personal_brand_feedback`, `generate_personal_brand_concept`, `personal_brand_command`, `plan_personal_brand_platform_variants`, `personal_brand_concept_to_content_item`, `personal_brand_website_founder_content`, `request_personal_brand_digital_twin`) → `requireSuperAdmin(admin)` before each write | Founding Beta / Stage B-G — real image+copy generation via existing Cloudflare/Lily engines; video content only ever gets a real script/storyboard (`renderingAvailable:false`), never a rendered file, UNLESS `uses_ai_clone` routes through the real HeyGen+ElevenLabs adapter (Stage G — see `_shared/marketing-clone-provider-heygen-elevenlabs.js`); `request_clone_enrollment`/`preview_clone_profile` genuinely call HeyGen (avatar) + ElevenLabs (voice) once `HEYGEN_API_KEY`/`ELEVENLABS_API_KEY` are both set in the environment, else fall back to the not-live stub exactly as before; `upload_clone_reference_photo` hosts a reference photo in the public website-media bucket so HeyGen has a real URL to fetch it from; `clone_job_status` checks the webhook-updated `marketing_clone_video_jobs` row first (see `heygen-webhook` above), falling back to a live HeyGen poll exactly as before if no webhook has landed yet (read-only, no `requireSuperAdmin` gate, same as `list_clone_consent`); `set_content_disclosure` records AI-content provenance on a platform variant and recomputes `ai_disclosure_required` from `_shared/creative-ai/disclosure-policy.js`'s per-platform policy table — never trusts a caller-supplied requirement flag directly; `run_publishing_queue` now runs a fail-closed disclosure gate before every publish attempt (content whose disclosure is required but not yet applied settles straight to `failed`, classified `fatal` — retrying can't fix a missing disclosure); the publishing queue (retry/backoff/dead-letter) is real, but every publish attempt still fails honestly (`social_provider_not_live`) since none of the 7 social platforms has a live, approved adapter — `connect_platform` reports whether OAuth env credentials are configured (all false today) but never fabricates an authorize redirect; `analytics_summary`/`list_insights` are real machinery over honestly-empty data until Stage E actually publishes; `evaluate_ab_experiment` never declares a winner below a real minimum sample size per variant; `plan_month`/`enqueue_publish` are idempotent. **Personal Brand Studio** (new this pass — `_shared/personal-brand-memory.js`, `_shared/creative-ai/personal-brand-*.js`): a florist's own presentation preferences (clothing/environment/lighting/framing/personality — explicitly NOT health/religion/political/orientation/ethnicity, which have no category to even write into), learned via the same explicit-writes-immediately/inferred-promotes-after-repetition engine as Brand Brain/AI Style Memory; `personal_brand_command` is Lily's real LLM-classified (never regex-only) command entrypoint — mode selection, standing memory statements ("remember this"/"don't do this again"), and founder-concept generation all in one call; `generate_personal_brand_concept`/`personal_brand_command` persist to `ai_generated_assets` as `founder_concept`, grounded per-shop (profile/traits/founder story), never a hard-coded example; reference-photo consent is three independent booleans (store/image-generation/avatar-generation), never one blanket checkbox, and revoke is a status change on the same row (audit trail preserved) while delete is a separate, real hard-delete action; `personal_brand_concept_to_content_item` hands an approved concept to the EXISTING `marketing_content_items`/`generate_content`/disclosure/publishing pipeline unmodified; `request_personal_brand_digital_twin` only ever kicks off a render through the same `selectCloneProvider` router `preview_clone_profile` already uses, gated by `isDigitalTwinUseAuthorized()` against the existing `marketing_clone_consent` grant (never authorizes off a caller-supplied flag) — NOT LIVE without `HEYGEN_API_KEY`/`ELEVENLABS_API_KEY` configured, exactly like every other clone action. Admin UI: `public/marketing-studio-admin.js` (mounted from the "Marketing Studio" nav entry in `admin.html`) — status, AI Clone enrollment/preview/consent, and a Personal Brand Studio panel (profile, "Ask Lily" command box, reference photo upload/revoke/delete); content planning/calendar/publishing-queue/analytics/media-output-planner have no admin surface yet. **Lily + Digital Twin integration (this pass):** `personal_brand_command`'s classify/memory/generate/digital-twin-kickoff logic and `request_personal_brand_digital_twin`'s consent-verification/render-kickoff logic were both extracted into `_shared/creative-ai/personal-brand-service.js` (`runPersonalBrandCommand()`, `requestDigitalTwinGeneration()`) so `lily-ai.js`'s normal chat dispatcher calls the exact same functions in-process — never a second, duplicated implementation. `runPersonalBrandCommand()` also now resolves "use my Digital Twin"/"use my voice" from the SAME message via `findActiveDigitalTwinGrant()` (the shop's active, not-revoked consent + whichever of its avatar/voice profiles are actually `'ready'` — avatar and voice resolve independently) and "don't use my voice" always forces `voiceProfileId:null` even when avatar use is requested in the same turn. |
...
| `heygen-webhook` | NOT LIVE — PROVIDER CONFIGURATION REQUIRED until `HEYGEN_WEBHOOK_SECRET` is set and a real HeyGen endpoint is registered; HMAC-SHA256 signature required (`Heygen-Signature`/`Heygen-Timestamp` headers, `_shared/creative-ai/heygen-webhook-verify.js`), every event recorded to `marketing_webhook_events` with a deterministic idempotency key before any job-state change, correlates to `marketing_clone_video_jobs`. **Digital Twin result lifecycle (this pass):** a real completion now runs through `_shared/creative-ai/digital-twin-finalization.js`'s `finalizeDigitalTwinJob()` — the ONE canonical, idempotent path both this webhook and `marketing-studio.js`'s `clone_job_status` polling fallback call, so a webhook/poll race or duplicate delivery can never create a second `ai_generated_assets` row or double-record the master-generation cost (both converge on `applyWebhookStatusUpdate()`'s existing safe-transition guarantee — a job already terminal is never re-finalized). A genuine first-time completion validates the reported output (a real `https://` URL, never trusted merely because the payload says "completed"), re-checks the original consent grant (a video generated under since-revoked consent still becomes a real, auditable asset — cost was genuinely incurred — but is never auto-surfaced into the publish-eligible content queue), records the cost exactly once, and — when the request named a target platform — creates a real `marketing_content_items`/`marketing_platform_variants` row in `status:'in_review'` with disclosure metadata pre-computed from the existing per-platform policy engine, never auto-approved. |
