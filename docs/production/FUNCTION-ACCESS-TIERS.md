...
| `marketing-studio` | `super_admin` only — `platformAdmin(event, ["super_admin"])`, additionally gated behind the `MARKETING_STUDIO` feature flag (default off) | Brand Brain update/forget/reset, `plan_month` (monthly content planning), `approve_content` (review decision) → `requireSuperAdmin(admin)` before each write | Founding Beta / Stage B-C foundation only — no content generation, publishing, or AI Clone execution lands here yet; `status` action reports every social/AI provider as not-live until Stage D/E connect real credentials; `plan_month` is idempotent per shop+month |
