...
This document is that line. As of this writing, half of the trigger exists: the Command Center support inbox has a "Request Claude Code fix" button (`support-request-fix` action in `admin-command-center.js`) that a platform owner clicks per ticket — that's the human-authorization step this policy requires before any investigation starts. What it does when clicked is package the ticket, recent client errors from that shop, and a pointer to this policy, and POST it to `CLAUDE_CODE_FIX_WEBHOOK_URL` if one is configured. **No endpoint is configured by default, and none is provided by this codebase** — until an owner sets that variable to a real receiving service, clicking the button records the request on the ticket and sends it nowhere. There is still no automatic ticket → agent pipeline, and no connection at all from Lily's florist-facing chat. **Whoever configures or builds the receiving end of that webhook must wire it to this policy, not invent new rules at build time.** Until then, this document governs any agent (Claude Code, specifically) working in this repo under any circumstance, including a developer just asking for a fix in conversation.
