...
export function stripeApplicationFeeCents(amountDollars, { relayPercent } = {}) {
...
export function canInitiateWirePayment(wire, shopId) {
  ...
  const relayPercent = wire.relay_fee_percent ?? wire.metadata?.relay_fee_percent;
  const settlement = computeWireSettlement(amount, {
    relayPercent: relayPercent != null ? Number(relayPercent) : undefined,
  });
  if (settlement.fulfilling_shop_payout < 0.5) {
    return { ok: false, error: "Partner payout must be at least $0.50 for card payment." };
  }
  return { ok: true, amount: settlement.fulfilling_shop_payout, settlement };
}
