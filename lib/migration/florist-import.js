...
const HEADER_ALIASES = {
  name: ["name", "product name", "item name", "customer name", "full name", "flower", "item"],
  ...
  quantity: ["quantity", "qty", "on hand", "stock"],
  unit: ["unit", "uom"],
  cost: ["cost", "wholesale", "unit cost"],
  order_number: ["order number", "order #", "order id", "invoice"],
  delivery_date: ["delivery date", "due date", "ship date"],
  status: ["status"],
...
export function parseCsvText(text, { entity = "products" } = {}) {
  ...
  for (let i = 1; i < lines.length; i += 1) {
    ...
    if (entity === "customers") {
      ...
      const name = pick("name");
      if (!name) {
        errors.push(`Row ${i + 1}: missing inventory name`);
        continue;
      }
      rows.push({
        name,
        category: pick("category") || "Flowers",
        quantity: Math.max(0, Number(pick("quantity") || 0)),
        unit: pick("unit") || "stems",
        cost: Math.max(0, Number(pick("cost") || 0)),
      ...
      rows.push({
        customer_name: name,
        delivery_date: pick("delivery_date") || null,
        total: Math.max(0, Number(pick("price") || 0)),
        status: pick("status") || "PENDING",
        occasion: pick("occasion") || null,
        notes: pick("description") || null
      });
    } else {
      const name = pick("name");
      if (!name) {
