/**
 * Florisyn Master Floral Library — bundled JSON (Netlify-safe).
 * Do not use fs + fileURLToPath(import.meta.url) here; esbuild breaks __dirname in functions.
 *
 * Started as "Everyday only" (batch1/batch2, ~100 items) — the name stuck
 * even after occasion-specific batches (Funeral, Sympathy, Wedding, etc.)
 * were added below. Those batches existed fully generated on disk with
 * real images since 2026-08-11 but were never actually merged in here, so
 * every florist's Floral Library was "Everyday" only regardless of the
 * occasion filter the UI already offered — filed for the funeral/sympathy
 * gap a user reported after comparing to a real florist site.
 */
...
import funeral from "../../public/data/floral-library-funeral.json" with { type: "json" };
import sympathy from "../../public/data/floral-library-sympathy.json" with { type: "json" };
import birthday from "../../public/data/floral-library-birthday.json" with { type: "json" };
import wedding from "../../public/data/floral-library-wedding.json" with { type: "json" };
import congratulations from "../../public/data/floral-library-congratulations.json" with { type: "json" };
import getWell from "../../public/data/floral-library-get-well.json" with { type: "json" };
import hydrangeas from "../../public/data/floral-library-hydrangeas.json" with { type: "json" };
import loveRomance from "../../public/data/floral-library-love-romance.json" with { type: "json" };
import newBaby from "../../public/data/floral-library-new-baby.json" with { type: "json" };
import plants from "../../public/data/floral-library-plants.json" with { type: "json" };

const ULTRA_REALISTIC_IMAGE_STANDARD = "ultra_realistic_professional_floral_photography";

const OCCASION_BATCHES = [
  funeral,
  sympathy,
  birthday,
  wedding,
...
function mergeArrangements() {
  const merged = [
    ...(batch1.arrangements || []),
    ...(batch2.arrangements || []),
    ...OCCASION_BATCHES.flatMap((b) => b.arrangements || []),
  ];
